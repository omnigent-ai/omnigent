...
_KEY_PROVIDER_ENDPOINT: dict[str, _VendorEndpoint] = {
    ...
    "orcarouter": _VendorEndpoint("https://api.orcarouter.ai/v1", CHAT_WIRE_API),
...
def add_menu_options() -> list[AddOption]:
    ...
    :returns: The ordered :class:`AddOption` list backing the add menu.
        ...
        (Gateway, OpenRouter, OrcaRouter) follow alphabetically, and
        Databricks sits just above the catch-all "Other provider" at the
        bottom. Scoped to one family this collapses to: API key →
        subscription → Gateway [→ OpenRouter → OrcaRouter] → Databricks
        [→ Other].
    """
    ...
    return [
        ...
        # Cross-vendor extras, alphabetical (Gateway, OpenRouter, OrcaRouter).
        _opt(
        ...
        # OrcaRouter is absent from the MLflow catalog that backs "Other
        # provider", so a preset entry is the only way to reach it by name.
        _opt(
            "OrcaRouter — API key",
            "One key, many models (orcarouter.ai).",
            KEY_KIND,
            provider="orcarouter",
        ),
