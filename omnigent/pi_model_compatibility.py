...
# Anthropic Claude models support extended thinking via Pi's thinking level
# controls. ``reasoning: True`` in the model entry is how Pi decides whether to
# show the thinking level picker and honour ``--thinking`` / ``setThinkingLevel``.
...
def pi_model_json_entry(model: ModelEntry) -> PiModelEntry:
    ...
    if pi_model_is_reasoning(model.id) or any(fragment in model.id.lower() for fragment in PI_CLAUDE_THINKING_MODEL_FRAGMENTS):
