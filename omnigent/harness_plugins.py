...
_BUILTIN_CAPABILITIES: dict[str, HarnessCapabilities] = {
    ...
    # Headless OpenCode SDK harness. See omnigent/inner/opencode_executor.py.
    # Drives a per-conversation ``opencode serve`` subprocess over HTTP/SSE via
    # the ``opencode-ai`` Python SDK — the chat-first counterpart to the
    # terminal-first ``opencode-native`` harness above.
    "opencode": _C(
        _IM.CLI_SUBPROCESS,
        _EL.SSE_PERMISSION,
        _RS.COLD_ONLY,
