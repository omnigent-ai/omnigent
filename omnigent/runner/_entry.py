...
def _make_auth_token_factory(
    ...
    *,
    ...
    if token_command:
        # Security: ``OMNIGENT_DATABRICKS_TOKEN_COMMAND`` is arbitrary
        # command execution (``shlex.split`` + ``subprocess.run``). It
        # MUST only originate from deploy configuration (e.g. the CoDA
        # app's ``app.yaml`` env block) and can NEVER be set from
        # agent/session/host-supplied config. The deployer is the trust
        # boundary; the runner is the execution boundary. This var is on
        # the propagated-env allowlist (connect.py) so host-spawned runners
        # inherit it, but only the host owner controls that env.
...
def create_app(
    ...
    if os.environ.get(RUNNER_PREFER_BINDING_TOKEN_MINT_ENV_VAR) and binding_token:
        _callback_headers[RUNNER_TUNNEL_TOKEN_HEADER] = binding_token
    server_client = httpx.AsyncClient(
        base_url=server_url,
