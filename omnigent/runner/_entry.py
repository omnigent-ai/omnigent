...
def _make_auth_token_factory(
    ...
    *,
    ...
    # Reused Databricks SDK auth, resolved once on first use and reused for
    # the life of the factory: the SDK serves the minted OAuth token from
    # its in-memory cache and only re-runs the Databricks CLI (~0.5s) when
    # the token nears expiry, instead of shelling out on EVERY runner->AP
    # request. The token source re-resolves itself when a mint through the
    # reused auth fails (e.g. a CLI upgrade deleted the binary path the SDK
    # baked at construction), so a long-lived runner recovers instead of
    # failing closed for the rest of the session.
