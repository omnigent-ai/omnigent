            ...
            if harness_name == "claude-native":
                ...
                async def _codex_build_context(ctx: NativeLaunchContext) -> NativeLaunchContext:
                    ...
                    if bundle_dir is not None and spec is not None:
                        _ensure_orchestrator_skills_in_bundle(bundle_dir, spec)
                    codex_cwd = await _session_runtime_cwd(session_id)
                    codex_additional_directories = await _session_additional_directory_paths(
                        session_id,
                        cwd=codex_cwd,
                    )
                    ...
                    return dataclasses.replace(
                        ...
                        runtime_cwd=codex_cwd,
                        additional_directories=tuple(
                            str(path) for path in codex_additional_directories
                        ),
                    )
