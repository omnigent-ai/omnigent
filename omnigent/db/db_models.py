...
class SqlIntegrationConnection(OmnigentBase):
    """
    SQLAlchemy model for the ``integration_connections`` table.
    ...
    (:attr:`secret_enc`, Fernet ciphertext via
    :class:`omnigent.server.secretbox.SecretBox`) so any provider's secret
    shape fits without a schema change — plaintext never touches the database.
