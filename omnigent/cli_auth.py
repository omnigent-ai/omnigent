...
def databricks_request_headers(
    ...
    *,
    ...
    # workspace request still carries a key (routed by the sharding layer rather
    # than depending on its implicit workspace-id default). Two ordered
    # fallbacks, both host_ids:
    ...
    #      stays unkeyed (→ workspace-id default). Gated on the workspace mount
    #      below so the file read only happens for requests that could use it.
    # Only the workspace-hosted server runs the replica-sharding layer that reads
    # the header; a local / self-hosted server is single-replica and would just
    # log a header it ignores — so gate emission (and the CLI identity lookup) on
    # the mount. Callers never reason about the deployment; a new RPC routed
    # through this builder is keyed automatically.
    ...
    if host_id and on_workspace_mount:
...
def open_server_client(
    ...
    *,
    ...
    :func:`databricks_request_headers` in for you, so a request to the
    workspace-hosted mount automatically carries the org-id and slice-key
    routing headers (and any dev/test selectors) — and a request to a local /
    self-hosted server carries none. Callers never reason about the deployment;
    a new server RPC opened through this factory is routed correctly by
    construction, which is why it exists rather than each site building headers
    by hand.
    ...
    :param host_id: The host a request is scoped to, forwarded to
        ...
        on the workspace mount and otherwise falls back to the runner's own
