...
def _read_open_session_binding(
    *,
    ...
    can_open_native = type(permission_level) is int and permission_level >= LEVEL_OWNER
    if native_agent is not None and permission_level is None:
        server_info = _host_http_json(base_url=base_url, method="GET", path="/v1/info")
        if server_info.status_code == 0 or server_info.status_code >= 500:
            raise _OpenTransientError(
                f"Couldn't verify native terminal access: {_host_error_text(server_info.body)}"
            )
        can_open_native = (
            server_info.status_code == 200
            and isinstance(server_info.body, dict)
            and server_info.body.get("single_user") is True
        )
    if native_agent is not None and not can_open_native:
        import shlex
        ...
        def _send_with_routing_fallback(
            ...
            *,
            ...
            if (
                ...
                if demotion_key is not None and 200 <= result.status_code < 300:
                    with _host_http_headers_lock:
