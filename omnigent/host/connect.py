...
# registry record. Cheap local-file read; a few seconds is prompt enough to
# retire a superseded daemon without polling churn.
_LIFECYCLE_POLL_INTERVAL_S = 5.0
...
class HostProcess:
    ...
    def __init__(
        ...
        # record. When the record is deleted/reassigned the monitor cancels the
        # in-flight serve and sets _lifecycle_lost so run() breaks and tears
        # down cleanly instead of lingering as a stale daemon.
        self._lifecycle_lock = lifecycle_lock
        self._lifecycle_task: asyncio.Task[None] | None = None
        self._lifecycle_lost = asyncio.Event()
        # The current _connect_and_serve task, so the lifecycle monitor can
        # cancel it to trigger a clean shutdown.
    ...
    async def _lifecycle_monitor_loop(self) -> None:
        ...
        means this process is stale. It then cancels the in-flight serve and
        sets ``_lifecycle_lost`` so :meth:`run` breaks and its ``finally`` reaps
        runners and releases the lock.
        ...
        try:
            while True:
                if self._lifecycle_lost.is_set():
                    break
                try:
                    # Run the serve as a task so the lifecycle monitor can
                    # cancel it to trigger a clean shutdown.
                    self._serve_task = asyncio.create_task(
                        self._connect_and_serve(), name="host-serve"
                    )
                    await self._serve_task
                    backoff = _RECONNECT_BASE_S
                except (KeyboardInterrupt, asyncio.CancelledError):
                    # Either the lifecycle monitor cancelled the serve, or run()
                    # itself was cancelled (SIGINT / shutdown) while awaiting it.
                    # The serve task is a child, so cancel + await it here so it
                    # can't keep serving frames during teardown.
