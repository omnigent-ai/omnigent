...
def _remote_headers(
    ...
    *,
    ...
    :param host_id: The host a request is scoped to, or ``None`` when the
        ...
        request that keys off the runner-env host_id). Defaults to ``None``
        for probes and health checks; pass the request path's host when it
        has one so the request reaches the replica holding that host's tunnel.
...
class _DatabricksTokenAuth(httpx.Auth):
    ...
    def _sdk_token(self) -> str | None:
        ...
        if not self._sdk_auth_resolved:
            workspace_host = load_databricks_workspace_host(self._server_url) if self._server_url else None
            try:
...
async def _prepare_chat_session_via_daemon(
    *,
    ...
    async with open_daemon_client(base_url, headers, host_id, auth=auth, timeout=timeout) as client:
        if progress is not None:
