...
_WATCHED_BRIDGE_FILES = (
    ...
    MESSAGE_DELTAS_FILE,
    # Observer-hook stderr; the loop relays new bytes into the runner log, so
    # a hook failure must wake the gate rather than wait for the resync.
    OBSERVER_HOOK_STDERR_FILE,
)
    ...
    async with (
        ...
        while True:
            try:
                ...
                # The deadline cancelled the stalled await mid-iteration;
                # cursor state advances only after successful posts, so
                # resuming retries the interrupted step after a full re-poll.
                fingerprint = None
                _logger.warning(
