"""KimiAcpExecutor: run agents through Kimi Code's ACP mode.

Spawns Kimi Code (``kimi acp``) as a subprocess and communicates via the
...
Kimi Code manages its own agent loop, tool execution, context window, and
compaction internally.  This executor translates the ACP event stream into
Omnigent ExecutorEvents.

Requirements:
    The ``kimi`` CLI (v0.20+) must be installed and on PATH.
"""
...
# ACP error code kimi maps to a filesystem "not found" (ENOENT) when a
# delegated ``fs/read_text_file`` fails — kimi's AcpFileSystemService special-
...
class _AcpRequestError(Exception):
    ...
    :meth:`KimiAcpExecutor._respond_to_agent_request` can build the error reply
...
def _looks_like_missing_file(message: str) -> bool:
    ...
    onto kimi's ENOENT code so the model sees "file not found" rather than a
...
# Notification (no ``id``) sent *to* the agent to cancel the in-flight turn
# without tearing down the session — the ACP-native interrupt.
_AGENT_NOTIFY_SESSION_CANCEL = "session/cancel"
...
# Kimi Code emits its chain-of-thought over ACP as ``agent_thought_chunk``
# (unlike qwen, which surfaces no thought stream). Mirror it as a
# ReasoningChunk so the web UI paints a reasoning block.
_UPDATE_AGENT_THOUGHT_CHUNK = "agent_thought_chunk"
_UPDATE_TOOL_CALL = "tool_call"
_UPDATE_TOOL_CALL_UPDATE = "tool_call_update"

# How long (seconds) to wait for kimi to respond to a JSON-RPC request
...
class KimiAcpExecutor(Executor):
    """Executor that drives Kimi Code via its ACP (``acp``) mode.

    Spawns a ``kimi acp`` subprocess and manages sessions through the
    ...
    def __init__(
        ...
        """Initialize the Kimi Code executor.

        :param cwd: Working directory for the kimi subprocess.  When
            ``None``, the subprocess inherits the caller's cwd.
        :param os_env: Environment / sandbox spec. When its ``sandbox`` is not
            ``"none"``, the whole ``kimi`` process tree is wrapped in the
            platform sandbox (bwrap/seatbelt) at spawn — see
            :meth:`_sandbox_launch_path` — so kimi's own file/shell tools are
            confined to the spec's read/write roots, not just gated by the
            permission policy.
        :param model: Model identifier to pass in ``session/new``.
        :param kimi_path: Absolute path to kimi CLI binary.
            Defaults to ``"kimi"`` (PATH lookup).
        """
        self._cwd = cwd or os.getcwd()
        self._os_env = os_env
        # Whether to advertise ``clientCapabilities.fs`` so kimi delegates file
        ...
        # whose path would diverge from the cwd the kimi subprocess actually
        # runs in, so delegating there would read/write the wrong directory.
        # When disabled, kimi falls back to its own file tools (see
        ...
        self._model = model
        self._kimi_path = kimi_path or "kimi"
        ...
        # Drains kimi stderr so a chatty CLI can't fill the pipe buffer
        ...
        # ACP session id assigned by kimi (returned in session/new response).
        self._session_id: str | None = None

        # Whether initialize has been sent already.
        self._initialized: bool = False

        # Whether kimi accepts ``image`` prompt blocks, learned from the
    ...
    async def _start_process(self) -> None:
        """Start ``kimi acp`` as an asyncio subprocess.

        The StreamReader limit is set to 16 MiB so that kimi's large
        ...
        # this the new process would skip ``initialize`` and kimi would
        ...
        env = os.environ.copy()
        # Resolve the path to spawn: the bare kimi binary, or a sandbox launcher
    ...
    def _sandbox_launch_path(self, spawn_env_names: Sequence[str]) -> str:
        ...
        ``os_env.sandbox`` requests confinement, wraps the kimi binary in the
        platform sandbox (``linux_bwrap`` / ``darwin_seatbelt``) so the *entire*
        kimi process tree — its built-in file tools and any shell child
        ...
        *allowed* tool call can't touch paths outside the sandbox.
        ...
        try:
            ...
            if not sandbox.active:
                return self._kimi_path
            # Best-effort sandbox mapping (only reached when os_env requests a
            # jail; the default os_env is unsandboxed). kimi ships as a single
            # curl-installed binary, so grant read access around its install
            # location and write access to its config dir (~/.kimi) and /tmp, or
            # it can't start inside the jail. The exact read/write set for a
            # sandboxed kimi is unverified — widen here if a jailed launch fails.
            kimi_dir = Path(self._kimi_path).resolve().parent.parent
            sandbox = with_additional_read_roots(sandbox, [kimi_dir])
            ...
            return create_exec_launcher(self._kimi_path, sandbox)
        except (OSError, ImportError, NotImplementedError) as exc:
            logger.warning("Could not apply sandbox for kimi; running unsandboxed: %s", exc)
            return self._kimi_path

    async def _read_stderr(self) -> None:
        """Continuously drain kimi stderr, logging each line at debug.

        With ``stderr=PIPE`` and no reader, a chatty ``kimi`` process can
        ...
        try:
            while True:
                ...
                if line:
                    logger.debug("kimi stderr: %s", line)
        except asyncio.CancelledError:
            # Expected on shutdown (close() cancels this task); exit quietly.
            pass
        except Exception as exc:  # noqa: BLE001
            logger.debug("kimi stderr reader stopped: %s", exc)

    async def _read_stdout(self) -> None:
        """Continuously read NDJSON lines from kimi stdout.
        ...
        try:
            while True:
                ...
                if not raw_line:
                    # EOF — the kimi subprocess exited (a crash mid-turn
                    ...
                    for fut in self._pending.values():
                        if not fut.done():
                            fut.set_exception(EOFError("kimi subprocess closed stdout"))
                    break
                ...
                try:
                    ...
                    logger.debug("kimi: non-JSON stdout line: %r", line[:200])
                    continue

                msg_id = msg.get("id")
                # Match a response by "id + no method": kimi's own requests
            ...
            logger.exception("kimi stdout reader error: %s", exc)
    ...
    async def _send(self, msg: dict[str, Any]) -> None:  # type: ignore[explicit-any]
        """Write one newline-terminated JSON message to kimi stdin."""
    ...
    async def _ensure_initialized(self) -> None:
        ...
        resp = await self._rpc(
            _AGENT_METHOD_INITIALIZE,
            {
                ...
                # Advertise fs delegation so kimi routes file reads/writes back
                # to us (executed via the OSEnvironment) rather than touching
                # disk directly. kimi's AcpFileSystemService swaps in only when
                # the matching flag is true (cli.js: ``setupFileSystem``); both
                # false (no os_env / fork env) leaves kimi on its own tools.
                "clientCapabilities": {
                    "fs": {
        ...
        if "error" in resp:
            raise RuntimeError(
                f"kimi ACP initialize failed: {resp['error'].get('message', resp['error'])}"
            )
        prompt_caps = (
    ...
    async def _ensure_session(self) -> str:
        ...
        :returns: The session id string assigned by kimi.
        """
        ...
        if "error" in resp:
            raise RuntimeError(
                f"kimi ACP session/new failed: {resp['error'].get('message', resp['error'])}"
            )

        # Kimi Code assigns (possibly remaps) the session id — always use what
        # the server returns, not what we sent.
        result = resp.get("result", {})
        server_session_id = result.get("sessionId")
        if not server_session_id:
            raise RuntimeError(
                "kimi ACP session/new response missing sessionId: " + json.dumps(resp)[:200]
            )
    ...
    async def _respond_to_agent_request(self, request: dict[str, Any]) -> None:  # type: ignore[explicit-any]
        """Answer a server-initiated ACP request from kimi.

        kimi can drive the client mid-turn (e.g. permission prompts). A blanket
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          advertised (an os_env is configured; see :attr:`_fs_delegation`), kimi
          ...
          layer and the I/O flows through Omnigent rather than kimi touching
          disk directly. With delegation off, these never arrive (kimi uses its
          own tools) and would hit the ``method not found`` branch.
        - anything else — reply with a JSON-RPC ``method not found`` error
          rather than a bogus success, so kimi fails loudly instead of acting
          on empty data.
        ...
        # Log every server-initiated request so DEBUG reveals whether kimi
        # actually delegates permissions/fs to us (vs handling them itself).
        logger.debug("kimi agent request: method=%s id=%s", method, req_id)
        ...
        try:
            ...
            # forward its specific code/message so kimi maps it correctly.
            error = {"code": exc.code, "message": exc.message}
        except Exception as exc:  # noqa: BLE001
            logger.debug("kimi agent request %s failed: %s", method, exc)
    ...
    # ------------------------------------------------------------------
    # Filesystem delegation (kimi → client, when fs capability advertised)
    # ------------------------------------------------------------------
    ...
    async def _ensure_os_environment(self) -> OSEnvironment:
        ...
        :raises _AcpRequestError: When no usable os_env can be created — surfaced
            to kimi as an internal error rather than crashing the turn.
        """
    ...
    async def _handle_fs_read(self, params: dict[str, Any]) -> dict[str, Any]:  # type: ignore[explicit-any]
        ...
        :raises _AcpRequestError: On a missing path arg, a non-text/binary file,
            ...
            file so kimi raises the right error to the model).
        """
    ...
    def _extract_tool_call(params: dict[str, Any]) -> tuple[str, dict[str, Any]]:  # type: ignore[explicit-any]
        ...
        Kimi Code's payload carries a ``toolCall`` with ``_meta.toolName`` (e.g.
    ...
    async def _decide_permission(self, params: dict[str, Any]) -> bool:  # type: ignore[explicit-any]
        ...
        if policy_eval is not None:
            ...
            try:
                ...
                logger.warning("kimi TOOL_CALL policy eval failed for %s: %s", tool_name, exc)
                action = None
            if action == "POLICY_ACTION_DENY":
                logger.info("kimi permission denied by policy: tool=%s", tool_name)
                return False
            if action == "POLICY_ACTION_ASK":
                if handler is None:
                    logger.warning(
                        "kimi TOOL_CALL policy ASK with no elicitation handler; denying tool=%s",
                        tool_name,
                    )
                    return False
                allowed = bool(await handler(tool_name, tool_input))
                logger.info(
                    "kimi permission %s by user (policy ASK): tool=%s",
        ...
        if handler is not None:
            allowed = bool(await handler(tool_name, tool_input))
            logger.info(
                "kimi permission %s by user: tool=%s",
                "allowed" if allowed else "denied",
                tool_name,
            )
            return allowed
        ...
        logger.debug("kimi permission allowed (no policy/elicitation wired): tool=%s", tool_name)
        return True
    ...
    def _accumulate_usage(  # type: ignore[explicit-any]
        ...
        Best-effort: Kimi Code's ACP adapter does **not** currently surface
        per-turn token usage over the wire (no ``_meta.usage`` on any
        ``session/update``), so in practice this is a no-op and the turn's usage
        stays empty. Retained defensively — if a future Kimi version starts
        emitting a ``{"usage": {"inputTokens", "outputTokens", "totalTokens",
        "cachedReadTokens"}}`` block (the field shape assumed here), usage will
        begin flowing without further changes. Summing across a turn's model
        calls matches actual cost (each API call bills its own full input).

        ``cachedReadTokens`` (if present) is split out into
        ``cache_read_input_tokens`` and kept out of ``input_tokens`` — cached
        tokens bill at a lower rate, so :func:`compute_llm_cost` expects the
        non-cached remainder in ``input_tokens`` (mirrors the codex executor).

        :param acc: The running per-turn accumulator (wire-shape keys), mutated
            in place. Absent any usage update it stays empty.
    ...
    def _text_from_blocks(
        ...
        *,
        ...
        - ``input_image`` → handled out-of-band as a real ACP ``image`` prompt
          ...
          set (kimi lacks image capability), so the image isn't silently lost.
        ...
        :param emit_image_marker: When ``True``, append a ``[attached image:
            …]`` marker for each ``input_image`` (used when kimi can't accept a
        ...
        for block in blocks:
            ...
            if btype in ("input_text", "output_text", "text"):
                ...
                # Only when kimi can't take a real image block (capability off):
            ...
            # see the harness integration guide).
        return "\n".join(parts)
    ...
    def _history_prefix(cls, prior: list[Any]) -> str:  # type: ignore[explicit-any]
        ...
        ``kimi acp`` process, or after a ``Session not found`` reset) kimi
    ...
    async def run_turn(
        ...
        tools: list[Any],  # type: ignore[explicit-any]  # noqa: ARG002 — kimi runs its own tool registry; param required by the Executor interface
        system_prompt: str,
        config: ExecutorConfig | None = None,  # noqa: ARG002 — unused; required by the Executor interface
    ) -> AsyncIterator[ExecutorEvent]:
        """Run one turn of the Kimi Code agent loop via ACP.
        ...
        :param tools: Tool specs (not passed directly to Kimi Code; Kimi Code uses
        ...
        """
        try:
            # Lazily boot the subprocess. A missing/unspawnable ``kimi`` binary
        ...
        for idx in range(len(messages) - 1, -1, -1):
            ...
            if role == "user":
                ...
                if isinstance(content, str):
                    ...
                    # Forward attached images as real ACP image blocks when kimi
        ...
        # thread — kimi otherwise only ever sees this turn's latest message.
        ...
        # spec ``prompt:``) never reach kimi and it runs uninstructed. The
        ...
        # still awaiting a reply — answer it instead of discarding it, or kimi
        ...
        # Per-turn token usage, summed across kimi's per-call usage emissions
        # (see _accumulate_usage). Stays empty when kimi reports none.
        turn_usage: dict[str, int] = {}

        while True:
            remaining = deadline - loop.time()
            if remaining <= 0:
                yield ExecutorError(message="Timeout waiting for kimi response", retryable=True)
                return
            ...
            if fut.done() and self._queue.empty():
                try:
                    ...
                    yield ExecutorError(message=f"kimi process error: {exc}", retryable=True)
                    return
                if "error" in response:
                    ...
                    return
                # Successful completion. Attach the per-turn token usage kimi
            ...
            if method == _CLIENT_NOTIFICATION_SESSION_UPDATE:
                ...
                if update_type == _UPDATE_AGENT_MESSAGE_CHUNK:
                    # kimi rides per-call token usage on an agent_message_chunk
                    ...
                    # Kimi's chain-of-thought — mirror as a reasoning block. Not
                    # accumulated into the final response text (it's thinking, not
                    # the answer), unlike agent_message_chunk above.
                    content = update.get("content", {})
                    thought = content.get("text", "") if isinstance(content, dict) else ""
                    if thought:
                        yield ReasoningChunk(delta=thought, event_type="reasoning_text")

                elif update_type == _UPDATE_TOOL_CALL:
                    # Kimi Code is executing a built-in tool — surface it as info.
                    tool_title = update.get("title", "tool_call")
                    logger.debug("kimi tool_call: %s", tool_title)
    ...
    async def interrupt_session(self, session_key: str) -> bool:  # noqa: ARG002 — per-process session
        """Interrupt the in-flight turn via the ACP ``session/cancel`` notification.

        Unlike a subprocess terminate, ``session/cancel`` cancels the current
        turn while keeping the long-lived ACP session (and its context) alive, so
        the next turn continues without a cold respawn + history replay.

        :returns: ``True`` if a cancel was sent, ``False`` if no live session.
        """
        if self._proc is None or self._proc.returncode is not None:
            return False
        ...
        if not session_id:
            return False
        try:
            # A JSON-RPC *notification* (no ``id``): the ACP spec forbids
            # notifications from returning errors, so we don't await a response.
            await self._send(
                {
                    "jsonrpc": "2.0",
                    "method": _AGENT_NOTIFY_SESSION_CANCEL,
    ...
    async def close(self) -> None:
        """Terminate the kimi subprocess and clean up."""
