"""Executor that bridges Omnigent web-chat turns into the native Cline TUI.

It does not launch ``cline`` — the ``omnigent cline`` wrapper already launched the
interactive TUI in the session terminal. Each web-UI turn injects the latest user
message into that same tmux pane (bracketed paste + Enter), so the message appears
in the running Cline TUI (and, since the web UI embeds the pane, in both
surfaces). Output is terminal-originated; the embedded terminal renders it live.
"""
...
from omnigent.cline_native_bridge import BRIDGE_DIR_ENV_VAR, inject_user_message
...
class ClineNativeExecutor(Executor):
    """Harness-side executor for ``omnigent cline`` web-UI turns.

    Injects each web-UI message into the running Cline TUI's tmux pane. Does not
    ...
    async def enqueue_session_message(self, session_key: str, content: Any) -> bool:
        """Inject a live steering message into the Cline terminal."""
        del session_key
    ...
    async def run_turn(
        ...
        """Inject the latest web-UI user message into the Cline TUI pane."""
        del tools, system_prompt, config
        text = _latest_user_text(messages, self._bridge_dir)
        if not text:
            yield ExecutorError(message="cline native turn had no user text to send")
            return
        try:
            async with self._inject_lock:
                await asyncio.to_thread(inject_user_message, self._bridge_dir, content=text)
...
def _bridge_dir_from_env() -> Path:
    """Resolve the cline-native bridge dir from the harness spawn env."""
    raw = os.environ.get(BRIDGE_DIR_ENV_VAR, "").strip()
    if not raw:
        raise RuntimeError(f"{BRIDGE_DIR_ENV_VAR} is required for the cline-native harness")
    return Path(raw)
...
def _content_to_text(content: Any, bridge_dir: Path) -> str:
    """Normalize executor content into text the Cline TUI receives.
    ...
    (``[Attached: <path>]``) so cline can open them with its Read tool — otherwise
