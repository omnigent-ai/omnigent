...
The spawn ``message`` is withheld from the router, so the routing signal is
``task_name`` plus session metadata only.
...
#: name, its task-name keys, no prompt (codex encrypts the spawn message on
#: some paths), its own spawn-model vocabulary, and the effort clamp plus
#: TUI routing notice a rewrite needs.
