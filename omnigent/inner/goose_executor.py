...
class GooseExecutor(Executor):
    ...
    async def _handle_fs_read(self, params: dict[str, Any]) -> dict[str, Any]:  # type: ignore[explicit-any]
        ...
        both optional → whole file) map onto :meth:`OSEnvironment.read`. The op
        is recorded onto the turn stream, and the content runs through
        result-phase policy before it reaches goose.
    ...
    async def _handle_fs_write(self, params: dict[str, Any]) -> dict[str, Any]:  # type: ignore[explicit-any]
        ...
        recorded onto the turn stream, and result-phase policy runs before the
        write so an explicit denial actually prevents it.
