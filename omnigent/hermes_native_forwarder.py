...
def _has_new_compaction(db_path: Path, hermes_session_id: str) -> bool:
    ...
    Returns ``True`` when any ``messages`` row has ``compacted=1``
    — hermes sets this flag on the post-compaction summary row
    after ``/compress``.
    """
    con = _connect_ro(db_path)
    if con is None:
...
async def _persist_hermes_compaction_item(
    ...
    *,
    ...
    Reads all ``active=1`` messages from the hermes state DB — after
    ``/compress`` these are the compacted state.
    """
            ...
            try:
                ...
                if hermes_session_id is not None:
                    ...
                    if await asyncio.to_thread(
                        ...
                        # Check for compaction: if any messages have
                        # compacted=1, hermes ran /compress. Persist a
                        # compaction item so transcript rebuild knows
                        # the boundary.
