...
The token is fetched on demand and never persisted in the sandbox: it lives
only as long as a single git operation / MCP connection, is refreshed
server-side (:func:`resolve_access_token`), and stops resolving the moment the
launch token expires or the session ends — so it is revocable and executor
agnostic. See ``designs/GITHUB_APP_SANDBOX_AUTH.md``.
"""
