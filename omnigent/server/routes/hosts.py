...
def create_hosts_router(
    ...
    *,
    ...
    async def set_host_permission(
        ...
        # Security: a ``manage`` grantee may grant ``view`` or ``use`` but
        # NOT ``manage`` — only the owner or an admin can delegate manage.
        # Without this, a manage grantee could spread manage access to anyone
        # (PR review: "manage can re-delegate manage").
        if level == HOST_LEVEL_MANAGE:
            is_owner = host is not None and host.user_id == actor
            is_admin = await asyncio.to_thread(_is_admin_caller, actor)
            if not is_owner and not is_admin:
                raise HTTPException(
                    status_code=403,
                    detail="only the host owner or an admin may grant manage access",
                )
        # The grantee must exist as a user row.  The session permission
        # store owns the users table; ensure the row (idempotent upsert).
        # Note: ``SqlHostPermission`` has no DB-level FK (Rule R032), so
        # this is an application-level referential integrity check, not
    ...
    async def delete_host_permission(
        ...
        .. note::
            Revocation is **prospective** — it blocks *new* session
            launches on this host but does not terminate a guest session
            already running. After launch, the runner authenticates by
            binding-token → ``runner_id`` match, which never re-consults
            host grants. To hard-stop a running guest session, use host
            shutdown (``POST /v1/hosts/{host_id}/shutdown``).
