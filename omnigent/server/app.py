...
def create_app(
    ...
    async def _lifespan(
        ...
        try:
            ...
            # Stop in-flight background fork materializations for the same
            # reason: a cancelled fork never announces its destination, so no
            # partial session is left behind.
            await cancel_fork_tasks()
