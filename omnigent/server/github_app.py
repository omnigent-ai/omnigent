...
class GitHubAppConfig:
    ...
    def from_env() -> GitHubAppConfig | None:
        ...
        The feature requires a client id, a client secret, a resolvable
        redirect URI (explicit, or derived from ``OMNIGENT_DOMAIN``), and
        a dedicated token-encryption key
        (``OMNIGENT_GITHUB_APP_TOKEN_ENC_KEY``). Missing any of these
        disables it. There is no fallback for the encryption key — a
        blank/absent key fails closed (returns ``None``) rather than
        silently deriving key material from another secret.
        ...
        # Token-at-rest encryption requires a dedicated key. There is no
        # fallback: without it the feature fails closed so stored tokens
        # are never encrypted with key material borrowed from another
        # secret (whose rotation would silently invalidate connections).
        token_enc_secret = os.environ.get("OMNIGENT_GITHUB_APP_TOKEN_ENC_KEY", "").strip()
        if not token_enc_secret:
            _logger.error(
                "GitHub App is configured (client id/secret and redirect are set) "
                "but disabled because OMNIGENT_GITHUB_APP_TOKEN_ENC_KEY is unset. "
                "Set it to a dedicated high-entropy secret to enable the integration."
            )
            return None
