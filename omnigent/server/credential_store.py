...
class IntegrationCredentialStore:
    ...
    def _to_entity(
        ...
        if with_secret:
            plaintext = self._cipher.decrypt(row.secret_enc)
            # ``None`` (wrong key / corrupt) degrades to "reconnect", not a 500.
            secret = json.loads(plaintext) if plaintext is not None else None
    ...
    def upsert(
        ...
        *,
        ...
        """
        now = now_epoch()
        secret_enc = self._cipher.encrypt(json.dumps(secret))
        ...
        with self._session() as session:
            ...
            if row is None:
                ...
                row.updated_at = now
            session.flush()
            return self._to_entity(row, with_secret=True)
    ...
    def update_secret(
        ...
        *,
        ...
        with self._session() as session:
            ...
            if row is None:
                return
            row.secret_enc = self._cipher.encrypt(json.dumps(secret))
