...
1. **Server YAML** (OSS / self-hosted): :func:`parse_sandbox_config`
   ...
   ``<data_dir>/config.yaml``)::
       ...
       sandbox:
         provider: modal          # databricks|lakebox|modal|daytona|boxlite
                                  # |cwsandbox|islo|e2b|openshell|kubernetes
   ...
   ``modal``, ``daytona``, ``cwsandbox``, ``islo``, and ``openshell``
   have managed-launch support; ``lakebox`` parses but rejects at launch
   — its launcher module ships only in the internal build, so an OSS
   deployment wanting Databricks Sandbox should name ``databricks``.
