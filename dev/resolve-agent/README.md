...
5. *(author path)* Re-runs the whole set to prove every live facet goes fail→pass
   ...
   **re-runs new tests with ambient vars set** to prove the fixtures are hermetic,
   ...
   (pytest-playwright `--video` for web/terminal facets, the VHS tape for CLI
   facets) so the before/after footage pair lands in the PR's Demo section and,
   for Linear bugs with a key available, on the ticket.
