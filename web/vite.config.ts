...
// turns it on.
const isDatabricksWorkspace = parsed.pathname.replace(/\/$/, "") === WORKSPACE_API_PATH;
if (isDatabricksWorkspace) process.env.VITE_DATABRICKS_WORKSPACE = "true";

if (useAuth) {
  const token = resolveToken(parsed.origin);
  if (token) {
    console.log(
      `[dev-proxy] target=${OMNIGENT_URL} (authenticated${isDatabricksWorkspace ? ", databricks-workspace" : ""})`,
    );
  } else {
    console.error(
