...
import { getStyleRoot, setEmbedScopeRoot } from "./host";
...
beforeEach(() => {
  setEmbedScopeRoot(styleRoot);
});

afterEach(() => {
  localStorage.clear();
  styleRoot.style.removeProperty(CSS_VAR);
  styleRoot.style.removeProperty("--test-mono-family");
...
describe("cssFontFamilyPreference — createCssFontFamilyPreference", () => {
  ...
  it("applies the family with the fallback stack appended", () => {
    pref.apply("Inter");
    expect(getStyleRoot()?.style.getPropertyValue(CSS_VAR)).toBe("Inter, var(--font-sans)");
  ...
  it("removes the property when applied empty (default)", () => {
    ...
    expect(getStyleRoot()?.style.getPropertyValue(CSS_VAR)).toBe("Inter, var(--font-sans)");
    pref.apply("");
    expect(getStyleRoot()?.style.getPropertyValue(CSS_VAR)).toBe("");
  });
  ...
  it("uses each preference's own fallback stack", () => {
    ...
    expect(getStyleRoot()?.style.getPropertyValue("--test-mono-family")).toBe(
