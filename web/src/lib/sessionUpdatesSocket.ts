 ...
 *
 * On the managed (embedded) UI, append `?omnigent_slice_key=<frozen modal host>`.
 * A browser WebSocket handshake can't set request headers, so the Dicer routing
 * key rides the query string (the same seam the terminal-attach WS uses). This
 * WS watches sessions across MANY hosts, so no single key is "correct" — it's a
 * NICE-TO-HAVE load/perf lever: keying by the modal host (the one backing most
 * of the user's sessions) lands the standing 4s rescan on the replica most
 * likely holding their sessions' tunnels, so the per-session status read hits
 * that pod's warm relay cache instead of the cross-replica DB fallback, and
 * spreads the standing connections finer than per-workspace. Correctness is
 * unaffected (the rescan reads are replica-safe).
 *
 ...
 * settled value. Omitted on standalone (no Dicer) and when no host was picked.
 *
