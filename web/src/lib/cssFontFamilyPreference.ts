...
// ride a CSS custom property on the style root: an unset property falls back
...
// no-ops with no style root, so boot-time restore on the server is harmless.
...
export interface CssFontFamilyPreferenceConfig {
  ...
  readonly key: string;
  /** CSS custom property set on the style root, e.g. `--ui-font-family`. */
...
export interface CssFontFamilyPreference {
  ...
  /**
   * Apply the family to the style root's CSS variable. An empty name removes
...
export function createCssFontFamilyPreference(
  ...
  return {
    ...
    apply: (name: string) => {
      ...
      root.style.setProperty(cssVar, `${normalized}, ${fallback}`);
    },
  };
}
