...
import {
  CONVERSATION_BLOCK_CACHE_LIMIT,
...
beforeEach(() => {
  ...
  useChatStore.setState({
    ...
    // Same reasoning for the transcript cache: it survives switchTo by
    // design, so a conversation seeded in one test would render from cache
    // in the next.
...
describe("chatStore — switchTo transcript cache", () => {
  it("renders a previously-viewed transcript synchronously on switch-back", async () => {
    seedSession("conv_a", [userMessage("resp_1", "hello A"), assistantMessage("resp_1", "hi A")]);
    seedSession("conv_b", [userMessage("resp_2", "hello B")]);

    await useChatStore.getState().switchTo("conv_a");
    await useChatStore.getState().switchTo("conv_b");

    // Read state BEFORE awaiting: the cached transcript must already be on
    // screen, with no loading blank, while revalidation is still in flight.
    const back = useChatStore.getState().switchTo("conv_a");
    const mid = useChatStore.getState();
    expect(mid.conversationId).toBe("conv_a");
    expect(mid.loadingConversation).toBe(false);
    expect(mid.blocks).toHaveLength(2);
    expect((mid.blocks[0] as UserMessageBlock).content).toEqual([
      { type: "input_text", text: "hello A" },
    ]);
  ...
  it("still revalidates over the network on a cache hit", async () => {
    seedSession("conv_a", [userMessage("resp_1", "hello A")]);
    seedSession("conv_b", []);

    await useChatStore.getState().switchTo("conv_a");
    await useChatStore.getState().switchTo("conv_b");
    const countA = (): number =>
      fetchMock.mock.calls.filter(([u]) => String(u).startsWith("/v1/sessions/conv_a/items"))
        .length;
    const before = countA();

    await useChatStore.getState().switchTo("conv_a");

    // Rendering from cache must not skip the refetch: the agent can commit
    // items while the user is away, and a frozen snapshot would drop them.
    expect(countA()).toBe(before + 1);
  });

  it("reconciles items committed while away without duplicating cached ones", async () => {
    seedSession("conv_a", [userMessage("resp_1", "hello A")]);
    seedSession("conv_b", []);
    ...
    const blocks = useChatStore.getState().blocks;
    expect(blocks.map((b) => b.ctx.itemId)).toEqual(["msg_resp_1_user", "msg_resp_1_asst"]);
  });

  it("orders items committed while away above an in-flight live preview", async () => {
    seedSession("conv_a", [userMessage("resp_1", "hello A")]);
    seedSession("conv_b", []);
    ...
    // While away, conv_a committed a new item and then started streaming a
    // fresh reply. The snapshot carries the committed item; the pump has
    // already pushed the live preview by the time the snapshot resolves.
    seedSession("conv_a", [
      userMessage("resp_1", "hello A"),
      assistantMessage("resp_2", "committed while away"),
    ]);

    const back = useChatStore.getState().switchTo("conv_a");
    const livePreview: TextDone = {
      type: "text_done",
      ctx: {
        agent: null,
        depth: 0,
        ...
        itemId: "live:resp_live",
      },
      fullText: "streaming…",
      hasCodeBlocks: false,
    };
    useChatStore.setState((s) => ({ blocks: [...s.blocks, livePreview] }));

    await back;

    // The older committed item must render ABOVE the still-streaming reply,
    // not below it.
    expect(useChatStore.getState().blocks.map((b) => b.ctx.itemId)).toEqual([
      "msg_resp_1_user",
      "msg_resp_2_asst",
  ...
  it("blanks and loads when switching to a conversation with no cached transcript", async () => {
    seedSession("conv_cold", [userMessage("resp_1", "hello")]);

    const p = useChatStore.getState().switchTo("conv_cold");
    const mid = useChatStore.getState();
    expect(mid.blocks).toEqual([]);
    expect(mid.loadingConversation).toBe(true);

    await p;
    expect(useChatStore.getState().blocks).toHaveLength(1);
  });

  it("never caches in-flight live preview blocks", async () => {
    seedSession("conv_a", [userMessage("resp_1", "hello A")]);
    seedSession("conv_b", []);

    await useChatStore.getState().switchTo("conv_a");
    // Model a mid-stream assistant preview: `live:*` blocks belong to the
    // in-flight turn, so they must never resurface on switch-back.
    const livePreview: TextDone = {
      type: "text_done",
      ctx: {
        agent: null,
    ...
    const back = useChatStore.getState().switchTo("conv_a");
    expect(useChatStore.getState().blocks.map((b) => b.ctx.itemId)).toEqual(["msg_resp_1_user"]);
    await back;
    expect(
      useChatStore.getState().blocks.some((b) => (b.ctx.itemId ?? "").startsWith("live:")),
    ).toBe(false);
  });

  it("restores the history window from cache so scroll-up still pages", async () => {
    const total = SESSION_HISTORY_PAGE_SIZE * 2 + 5;
    const fullItems = Array.from({ length: total }, (_, idx) =>
      userMessage(`resp_${idx.toString().padStart(4, "0")}`, `message ${idx}`),
    );
    ...
    await useChatStore.getState().switchTo("conv_big");
    const oldest = useChatStore.getState().oldestItemId;
    await useChatStore.getState().switchTo("conv_b");

    const back = useChatStore.getState().switchTo("conv_big");
    // Cursor + flag ride along with the cached blocks; without them the
    // instant render would silently lose the ability to page older turns.
    const mid = useChatStore.getState();
    expect(mid.hasMoreHistory).toBe(true);
    expect(mid.oldestItemId).toBe(oldest);
    await back;
  ...
  it("keeps scrolled-up pages through revalidation instead of collapsing to one", async () => {
    const total = SESSION_HISTORY_PAGE_SIZE * 2 + 5;
    const fullItems = Array.from({ length: total }, (_, idx) =>
      userMessage(`resp_${idx.toString().padStart(4, "0")}`, `message ${idx}`),
    );
    ...
    // Scroll up to page in a second page, so the cache holds two pages with a
    // cursor deeper than the initial window.
    await useChatStore.getState().switchTo("conv_big");
    await useChatStore.getState().loadMoreHistory();
    expect(useChatStore.getState().blocks).toHaveLength(SESSION_HISTORY_PAGE_SIZE * 2);
    const deepOldest = useChatStore.getState().oldestItemId;

    await useChatStore.getState().switchTo("conv_b");

    // Switch back and let the background revalidation settle. It must fold the
    // fresh initial window into the two cached pages, not overwrite them —
    // the transcript and the deep cursor both have to survive.
    await useChatStore.getState().switchTo("conv_big");
    const settled = useChatStore.getState();
    expect(settled.blocks).toHaveLength(SESSION_HISTORY_PAGE_SIZE * 2);
    expect(settled.blocks.map((b) => b.ctx.itemId)).toEqual(
      fullItems.slice(-SESSION_HISTORY_PAGE_SIZE * 2).map((it) => it.id),
    );
    expect(settled.oldestItemId).toBe(deepOldest);
    expect(settled.hasMoreHistory).toBe(true);

    // Paging older still works from the preserved cursor.
    await useChatStore.getState().loadMoreHistory();
    expect(useChatStore.getState().blocks).toHaveLength(total);
  });

  it("evicts the least-recently-viewed conversation past the cache limit", async () => {
    const ids = Array.from(
      { length: CONVERSATION_BLOCK_CACHE_LIMIT + 2 },
      (_, idx) => `conv_lru_${idx}`,
    );
    for (const id of ids) seedSession(id, [userMessage(`r_${id}`, `msg ${id}`)]);

    for (const id of ids) await useChatStore.getState().switchTo(id);

    // The first conversation aged out; the most recent ones are still warm.
    const oldest = useChatStore.getState().switchTo(ids[0]!);
    expect(useChatStore.getState().blocks).toEqual([]);
    expect(useChatStore.getState().loadingConversation).toBe(true);
    await oldest;

    const warm = useChatStore.getState().switchTo(ids[ids.length - 1]!);
    expect(useChatStore.getState().blocks).toHaveLength(1);
    await warm;
  });
});
