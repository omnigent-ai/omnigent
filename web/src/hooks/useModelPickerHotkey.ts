// ⌘⇧M (Ctrl+Shift+M on Win/Linux) opens the composer's model picker: the
// keyboard equivalent of typing bare "/model", reachable without leaving the
// keyboard.
//
// Chord trade-off: ⌘M alone minimizes the window on macOS and ⌥⌘M minimizes all
// windows, so both the single- and Option-modified forms are reserved by the
// OS. ⌘⇧M stays clear of those window bindings. In a browser tab ⌘⇧M is
// Chrome's profile menu, where preventDefault may not win, but it is free in the
// Electron shell (the primary surface). Like its sibling hotkeys it bails when
// focus sits in a surface that owns its own keys (xterm terminals, the Monaco
// editor).
...
/** True when the event is the model-picker chord: the platform command
 *  modifier + Shift + M, no Alt. */
export function isModelPickerHotkey(
  e: globalThis.KeyboardEvent,
  isMac: boolean = isMacPlatform(),
): boolean {
  // Platform command modifier + Shift, and explicitly not Alt, so the
  // Option-modified window-minimize chord (⌥⌘M) never matches.
  if (!hasCommandModifier(e, isMac) || !e.shiftKey || e.altKey) return false;
 ...
 */
export function useModelPickerHotkey(
  onOpen: () => void,
  enabled = true,
  isMac = isMacPlatform(),
): void {
  ...
  useEffect(() => {
    ...
    const handler = (e: globalThis.KeyboardEvent): void => {
      ...
      if (e.repeat) return;
      if (!isModelPickerHotkey(e, isMac)) return;
