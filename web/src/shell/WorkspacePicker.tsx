 ...
 * a Windows drive path (``C:\…`` / ``C:/…``), or a UNC share.
 */
...
export function basename(absolutePath: string): string {
  ...
  const sepIdx = lastSeparatorIndex(absolutePath.endsWith("/") || absolutePath.endsWith("\\")
    ? absolutePath.slice(0, -1)
    : absolutePath);
  const stripped =
    absolutePath.endsWith("/") || absolutePath.endsWith("\\")
...
export function listingFilter(
  ...
  const dirText = trimmed.slice(0, slash) || (isWindowsDrivePath(trimmed) ? trimmed.slice(0, 3) : "/");
