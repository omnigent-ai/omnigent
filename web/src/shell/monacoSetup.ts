...
//   2. Drive tokenization and theming from the custom Otto Ink light/dark themes
...
import { DARK_SYNTAX_THEME_NAME, LIGHT_SYNTAX_THEME_NAME, SYNTAX_THEMES } from "@/lib/syntaxTheme";
...
/**
 * Create the Shiki highlighter (Otto Ink light + dark) once and register its
...
export function ensureMonacoReady(): Promise<ShikiHighlighter> {
  if (!readyPromise) {
    readyPromise = createHighlighter({
      ...
      .then((hl) => {
        // Registers the Otto Ink themes under their ids so the editor's `theme`
 ...
 * @returns The registered Monaco theme id, e.g. `"omnigent-otto-ink-dark"`.
 */
export function resolvedThemeToMonaco(resolved: ResolvedThemeMode): string {
  return resolved === "dark" ? DARK_SYNTAX_THEME_NAME : LIGHT_SYNTAX_THEME_NAME;
}
