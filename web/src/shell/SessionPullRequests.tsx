...
export function SessionPullRequests({ conversationId }: SessionPullRequestsProps) {
  const { data } = useQuery({
    ...
    // Keep polling while the tab is unfocused and refetch on focus, so a PR the
    // agent just opened shows up promptly (GitHub's search index still adds a
    // short lag before a brand-new PR becomes discoverable).
