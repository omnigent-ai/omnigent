...
function MenuRowButton({
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-13 text-foreground hover:bg-accent",
        active && "bg-accent",
      )}
...
export function SlashCommandMenu({
  ...
  const sectionHeader = (label: string) => (
    <div className="px-2 pb-0.5 pt-1.5 text-2xs font-medium text-muted-foreground">{label}</div>
  );
