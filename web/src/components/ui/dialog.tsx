...
// Matches on component identity, so a header/footer is hoisted out of the
// scroller only when it is a DIRECT child of `DialogContent` (or sits in a
// keyless fragment, which `flattenFragments` opens first). A caller that wraps
// its footer in a `<div>`, a `<form>`, `React.memo`, or `forwardRef` keeps it
// inside the scrolling body — `ProjectSettingsDialog` shows the pattern that
// pins a footer from inside a `<form>`.
...
// boundary moving between renders. The footer keys off a constant instead: a
// conditional row appearing above it must not shift its key, or a focused
// footer button would remount and lose focus mid-interaction.
function keyed(nodes: React.ReactNode[], prefix: string | number): React.ReactNode[] {
  return nodes.map((node, index) => (
    <React.Fragment key={`${prefix}-${index}`}>{node}</React.Fragment>
  ));
}
...
function DialogContent({
  ...
  // Nothing between header and footer (a confirm dialog, or every middle child
  // conditional and currently off) renders no scroller at all, so there is no
  // empty div adding a stray `gap-4` between them.
  const hasBody = body.some((node) => node !== null && node !== undefined && node !== false);
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        // palette, which sits high on the screen — still wins through twMerge.
        className={cn(
