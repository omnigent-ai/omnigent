  ...
  return (
    <div className="absolute bottom-full left-0 z-10 mb-2 flex items-end gap-2">
      <div className="w-80 max-w-[calc(100vw-2rem)] shrink-0 overflow-hidden rounded-xl border border-border bg-popover shadow-lg">
        <div className="flex items-center justify-between gap-2 px-2 pb-0.5 pt-1.5 text-2xs font-medium text-muted-foreground">
          <span className="truncate">{currentDir ? `/${currentDir}` : "Workspace"}</span>
          <span className="shrink-0 text-3xs">↵ open · ⇥ attach</span>
        </div>
        {entries.length === 0 && loading ? (
          <div className="px-3 py-2 text-13 text-muted-foreground">Loading…</div>
        ) : (
            ...
            {entries.map((entry, i) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-13 text-foreground",
                ...
                >
                  ...
                  {isDir && (
                    <button
                      ...
                      className="flex shrink-0 items-center gap-0.5 rounded-md border border-border px-1.5 py-0.5 text-2xs text-muted-foreground hover:bg-accent hover:text-foreground"
