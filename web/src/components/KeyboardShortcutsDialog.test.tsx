...
describe("KeyboardShortcutsDialog", () => {
  it("advertises the session-search chord without taking the Print shortcut", () => {
    ...
    toggleViaHotkey();
    expect(
      screen.getByText("Find a session by name").closest("li")?.querySelector("kbd"),
    ).toHaveTextContent("Ctrl+Alt+S");
  });
  ...
  it("opens on the modifier+/ hotkey and lists one shortcut from each group", () => {
    ...
    expect(
      screen.getByText("Open previous session").closest("li")?.querySelector("kbd"),
    ).toHaveTextContent("Ctrl+[");
    expect(
      screen.getByText("Open next session").closest("li")?.querySelector("kbd"),
    ).toHaveTextContent("Ctrl+]");
