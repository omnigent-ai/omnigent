...
export function SystemMessageView({ message }: SystemMessageViewProps) {
  ...
  return (
    ...
    >
      ...
      {hasBody && open && (
        <div className="mt-0.5 max-h-64 max-w-full overflow-auto whitespace-pre-wrap rounded-[var(--radius-otto-sm)] border border-[var(--border-otto-hairline)] bg-muted/40 px-3 py-2 text-left text-xs text-muted-foreground [box-shadow:var(--elevation-otto-1)]">
          {message.body}
        </div>
      )}
    </div>
