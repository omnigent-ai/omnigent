...
describe("TerminalSession", () => {
  ...
  it.each([false, true])(
    "updates live accent selection without reconnecting (Codex palette: %s)",
    async (adaptCodexPalette) => {
      ...
      const { term } = session as unknown as { term: Terminal };
      try {
        expect(term.options.theme).toMatchObject(terminalTheme(false, "#a855f7"));
        const lightAnsi = term.options.theme?.extendedAnsi;
        session.setTheme(true);
        const darkAnsi = term.options.theme?.extendedAnsi;
        term.blur();
        root.style.setProperty("--primary", "#f97316");
        await Promise.resolve();
        expect(term.options.theme).toMatchObject(terminalTheme(true, "#f97316"));
        if (adaptCodexPalette) {
          expect(term.options.theme?.extendedAnsi).toEqual(darkAnsi);
          expect(darkAnsi).toBeDefined();
          expect(term.options.theme?.extendedAnsi).not.toEqual(lightAnsi);
        }
