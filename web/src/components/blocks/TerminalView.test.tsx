...
describe("terminal keybinding actions", () => {
  ...
  it("preserves terminal line editing, repeat, and IME ownership", async () => {
    const target = await renderTerminal();
    const send = terminalSessionMock.instances[0]!.sendInput;
    for (const [key, data] of [
      ["Backspace", "\u0015"],
      ["ArrowLeft", "\u0001"],
      ["ArrowRight", "\u0005"],
    ]) {
      expect(fireEvent.keyDown(target, { key, metaKey: true })).toBe(false);
      expect(send).toHaveBeenLastCalledWith(data);
      expect(fireEvent.keyDown(target, { key, metaKey: true, repeat: true })).toBe(false);
    }
    expect(send).toHaveBeenCalledTimes(6);
    send.mockClear();
    for (const key of ["Backspace", "ArrowLeft", "ArrowRight", "Enter"]) {
      const modifiers = key === "Enter" ? { shiftKey: true } : { metaKey: true };
      expect(fireEvent.keyDown(target, { key, ...modifiers, isComposing: true })).toBe(true);
      expect(fireEvent.keyDown(target, { key, ...modifiers, keyCode: 229 })).toBe(true);
      expect(fireEvent.keyDown(target, { key, ...modifiers, altKey: true })).toBe(true);
    }
