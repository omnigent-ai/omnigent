...
### 5.2 Subagent routing: the two-state switch, stamped at create, read per call

**5.2a** `subagent_routing_override` on the session is two-state: `"on"` routes subagent spawns, and `"off"` or unset leaves them to the harness. The create route stamps `"on"` on any session that starts on Smart Routing (its own `cost_control_mode_override == "on"`, the native/bundle auto path, or a routed parent), so the spawn gate reads one explicit switch instead of re-deriving routing from the session's or its parent's state. The in-session gear row shows this control for Claude Code sessions, for Codex sessions (native and SDK), and for Smart Routing sessions. A user can toggle it at any time, and the new value takes effect on the next spawn (plan §10 decision 5; `0fb7ea95`, web `1d030f22`, sticky per-harness default `2a415cf4`). This is the one routing control that *is* meaningful mid-session, and the reason is narrow: it changes only future spawns, so it promises nothing about the turn in flight (§2.1).

**5.2b — The row has exactly two options.** An earlier version added an explicit "Inherit" option because collapsing it onto the effective `on`/`off` made a re-pick of the displayed value a silent no-op (Radix sends no `onValueChange` for the value it already displays). The create-time stamp removes the third state at the source: unset now genuinely means Default, so the row collapses `"off"` and unset onto one option with nothing hidden behind it, and picking Default on an unset session correctly writes nothing.
