...
| --- | --- | --- | --- |
| `routing-mvp-v3` | `omnigent-routing-mvp-trim` | `d34ff216` | the shipping trim PR: create-time + composer-turn routing, subagent routing, CLI `--smart-routing -p`, bundle-agent gear config, GLM codex subagents, bundle scoping, two-state subagent gate + chip gate, per-harness gateway gating, MCP redirect actuation, `requested_model` honor. **Carries the row-95 defect** (arg-form `/model`, no picker helper) |
| `routing-mvp-v4` | `omnigent-routing-mvp-hookspike` | `799d5976` | merge of codex durability + **claude phase 2**: in-harness first-message routing for both harnesses. Has the row-95 **fix** (`inject_model_selection`, picker + session-only) and the row-100 fix, but **claude-native cannot launch** until row 93a is fixed |
...
| 47 | In-session **Subagent routing** row (Smart Routing / Default) | R0, gear → Subagent routing | the row is two-state: only `"on"` routes spawns, and `"off"` / unset both render **Default**. Sessions that start on Smart Routing are stamped `"on"` **at create** (`orchestration.py`, the create-path stamp), so the row never has to name an inherited state. `pickedSubagentRouting === undefined` still means untouched and reads through to the stored value, and opening the modal re-reads the switches via `refreshSessionOverrides()` because **nothing pushes a routing-switch change to the client**. Server accepts only `"on"`, `"off"`, `null` (`helpers.py`), and the spawn gate is `subagent_routing_override == "on"` (`subagent_routing.py`) | 🔄 **two-state collapse landed** — see area M row 105 | pre-2245f57d | — |
...
| 85 | An explicit `model` in the spawn arguments is **honored** when in-family | R0 codex session, spawn with an explicit `model` the harness can route to | the ask travels as `requested_model` and is honored for any spelling of an in-family arm | ✅ **live, re-proved 2026-08-04** — decision row rationale: `"Spawn requested system.ai.glm-5-2; honored — it is a routable arm for this session."`, `applied: true`, `scope: native_subagent`, and **no** `attempted_override` (correct: an honored ask stays out of that field). The honor path **short-circuits the router** — no `routes:select` at all between the session route (03:47:08) and `route-subagent … 200 OK 2235.6ms` (03:47:48). **Registry pointer correction:** the old evidence pointer "the `honored` audit line" is stale — `subagent_spawn_audit.jsonl` no longer exists on this branch (see the area-K note below). The DB decision row's `rationale` is now the ground truth | 2026-08-04 / d34ff216 | parent `e45e3de607ed42d78dfad7a19e1527ce` |
| 86 | A cross-family or unoffered explicit ask is **routed over** and recorded | R0 **claude** session, spawn asking for a non-claude arm (e.g. `haiku` cross-family, or a codex arm) | the spawn is routed within the claude family and the ask is recorded as `attempted_override=…` | 🟡 **unoffered-spelling half ✅ live; the cross-family (codex-arm-on-a-claude-parent) half INCOMPLETE.** Proven half: `{"model":"databricks-claude-sonnet-5","applied":true,"agent":"general-purpose","harness":"claude-native","scope":"native_subagent","attempted_override":"haiku"}` — a claude arm, never a codex one; the claude spawn menu is claude-only (`options=['claude-opus-5','claude-sonnet-5','claude-haiku-4-5',…]`, no gpt/glm); the subagent really ran (pane `SUBAGENT_SAID: durian`). Incomplete half: a second claude parent asking explicitly for `databricks-gpt-5-6-luna` had its session-scope row land but its `native_subagent` row was not yet written when the stream was stopped, and no `route-subagent` POST for it appears in the log | 2026-08-04 / d34ff216 | proven half `72ac17e67bbd41c7a628b6efc0041e17`; incomplete half `d404b46c0bae47128ab418bbbc584e33`; prior `e244e560` |
...
>
> - Rows **44, 85, 86** ask for audit-file evidence. Their ground truth on trim is
>   now the **DB decision row**, which carries both signals natively:
>   `rationale: "Spawn requested …; honored"` and `attempted_override: "haiku"`.
>   Those evidence pointers have been re-pointed above.
...
>   watcher line.
> - **Rows 45, 46 and recipe R8 are therefore not reproducible on the trim
>   branch at all** — the canary, the `subagent_routing_unenforced` warning and the
>   spawn-audit reconciliation they depend on were all removed. Their `c0b08f68`
>   evidence stands for the pre-trim branch only. **Decision owed:** either restore a
>   minimal enforcement signal on trim, or mark 45/46/R8 explicitly out of scope for
>   the shipping PR.
>
...
| 93 | **Phase 2 — claude bare-launch TUI routing** (block-and-replay) | R11 on claude | R2 pane capture: the block reason rendered in-pane, then the picker-based switch, then the replayed prompt, then the banner on the routed model. Actuator spec (**no fixed sleeps**): hook blocks and consumes the marker → confirm from the hook log/transcript, **not** pane text → drive the **picker** (never `/model <arg>`, row 95) → poll `capture-pane` for the `Switch model?` dialog → poll `read_claude_status_model` (`<bridge_dir>/context.json`) until it equals the target → `inject_user_message(...)` unchanged | 🔄 **product code LANDED on v4 and statically correct, but the live half is BLOCKED by row 93a.** The actuator `inject_model_selection` (`claude_native_bridge.py:3021`) implements the spec exactly — picker, session-only `s`, polled dialog, polled settle, no fixed sleeps — and is wired from `runner/turn_routing.py:1105-1145` and `inner/claude_native_executor.py:180`. But **no claude-native session can launch on v4 at all** (row 93a), so the pane experience, the one-turn replay and the banner were never observed live. S3 remains the only end-to-end mechanics evidence | 2026-08-04 / 799d5976 | source read; spike S3 |
| 93a | ❌ **BLOCKER (new, 2026-08-04)** — claude-native launch raises `TypeError` on `routing-mvp-v4` | launch any claude-native session on the v4 stack | `TypeError: augment_claude_args() got an unexpected keyword argument 'turn_router_dir'` at `omnigent/runner/native/orchestration.py:6111` in `_auto_create_claude_terminal` | ❌ **claude-native is 100 % dead on v4.** `orchestration.py:6121` passes `turn_router_dir=…`, added by **`9c64b35e`** ("make the blocked first prompt durable across runner crashes"), but the parameter was **never added to the callee** (`claude_native_bridge.py:1570`) — `git log -S turn_router_dir -- omnigent/claude_native_bridge.py` returns **zero commits** and the kwarg exists in exactly one place in the tree. **Not a merge artifact:** `9c64b35e` is not an ancestor of `4767328f` (siblings joined by merge `799d5976`) and its own tree already carries the mismatch, so v4 was broken for claude-native *before* the phase-2 merge. codex-native is unaffected (different launch function). **The kwarg is vestigial** — the route-turn hook is registered unconditionally at `claude_native_bridge.py:1353` and self-gates on the runner's bridge-dir advertisement. **Fix: delete `orchestration.py:6121`** — one line, no behavior change. Blocks the live half of rows 93, 96, 97, 103, and X16/X17 on claude | 2026-08-04 / 799d5976 | sessions `54b0e461db16420db9472583f44232d5`, `979162222bb74233a7149a81d8b4ee04` — 6 × TypeError per runner log, zero `tmux_socket=` lines, zero decision rows, repeated `session_has_no_registered_terminals` |
...
| 100 | **Pre-existing defect to fix independently** — the "Switch model?" dialog race | `inject_slash_command(auto_confirm=True)` on a session **with cached history** | the old code confirmed the dialog after a fixed `time.sleep(0.3)`, but it took **1.861 s** to render with history. The Enter is dropped, the dialog stays open, and the next `inject_user_message` fails with "terminal did not become ready within 30.0s". Fix: poll for the dialog | ✅ **FIXED on v4 (verified in source 2026-08-04)** — the fixed sleep is now `_confirm_tui_dialog` (`claude_native_bridge.py:2991-3019`), which **polls** `capture-pane` for `"Switch model?"`; `_wait_for_model_picker_applied` also handles the dialog inside its poll loop (`:3127-3128`). **No fixed sleeps remain on the decision path.** Still ❌ on **v3/TRIM**, which has none of this code — so the row stays open for the shipping PR and closes with the same port as row 95 | 2026-08-04 / 799d5976 (v4 fixed) | source read; prior S3 |
...
| 103 | **Claude first-message routing (phase 2)** — `omni claude --smart-routing` with **no** `-p`, then typing a prompt routes per turn in the TUI | `uv run --no-sync omnigent claude --server "http://127.0.0.1:$ROUTING_SERVER_PORT" --smart-routing` (no `-p`), then type the prompt into the TUI; score with R11 + R2 | one R1 turn-scope decision; the pane shows the block reason, then the picker switch, then the replayed prompt as ONE turn, then the banner on the routed arm; `~/.claude/settings.json` **unchanged** (row 95) | 🟡 **CLI half ✅, live half BLOCKED.** The CLI gate **is lifted** (2026-08-04): `--smart-routing` with no `-p` is accepted and prints `omnigent: Smart Routing is on for this session; your first message picks the model.` — `_require_smart_routing_prompt` (`cli.py:6144`) now returns `None` when `supports_in_harness_turn_routing(harness)`. The remaining half needs **row 93a fixed first** (`TypeError` kills every claude-native launch on v4), then the pane experience + the settings.json md5 check can be scored | 2026-08-04 / 799d5976 | `.omnigent-local/E2E_SWEEP.md` §7 |
...
| 109 | Python — routing unit slices | `uv run --no-sync pytest tests/cli/test_smart_routing_cli.py tests/test_native_initial_prompt.py tests/server/routes/test_native_smart_routing_create.py tests/server/test_turn_routing.py tests/test_codex_native_hook.py` | none known | ✅ evidence — TRIM smart-routing + subagent-routing + codex-hook + gateway slices (10 files): **316 passed / 0 failed**. SPIKE `test_turn_routing.py` + all 13 `*hook*` files: **343 passed / 1 failed**, the failure being the pre-declared v4 `assert 40 == 30` settings-timeout baseline (`test_claude_native_router_hook_settings.py::test_router_hook_registered_when_router_dir_set`). Zero collection errors anywhere | 2026-08-04 / d34ff216 + 799d5976 |
| 110 | Web | **`./node_modules/.bin/vitest run`** from `web/` — **not** `npx` (see status) | none known | ✅ evidence — TRIM **748 passed** across 6 files, SPIKE **698 passed** across 6 files, 0 failed. Covered `NewChatDialog` (**three** test files: `.test.tsx`, `.flow.test.tsx`, `.projectPrefill.test.tsx`), `ChatPage.composer`, `chatStore`, `routingDecision`. **Recipe correction:** the documented `npx vitest` line does **not** work non-interactively even with the real node bin prepended — nvm installs `npx`/`node` as zsh *functions* that shadow `PATH`, and the call recurses until `FUNCNEST` is exceeded. Call the `node_modules/.bin` shim directly | 2026-08-04 / d34ff216 + 799d5976 |
...
| Row(s) | State |
| --- | --- |
| **93a** | ❌ **NEW BLOCKER, 2026-08-04 — fix this first.** `claude-native` cannot launch at all on `routing-mvp-v4`: `TypeError: augment_claude_args() got an unexpected keyword argument 'turn_router_dir'` (`orchestration.py:6111`, arg passed at `:6121`, never added to the callee at `claude_native_bridge.py:1570`). Predates the phase-2 merge. **Fix: delete `orchestration.py:6121`** — the kwarg is vestigial. Until then the live halves of rows 93, 96, 97, 103 and claude X16/X17 are unreachable. |
...
| **95** | ❌ **ESCALATED 2026-08-04 — no longer just a phase-2 blocker.** The **shipping trim PR (`v3`)** rewrites the user's global `~/.claude/settings.json` on every routed claude model switch, caught live during the sweep. v3 has no picker helper at all; v4's `inject_model_selection` is the fix and needs porting. Separately `runner/app.py:3955` still uses the arg form on **both** branches. Also unaudited: `cursor_native_bridge.py:781`, `kiro_native_bridge.py:751`. |
| 96 | **Product blocker** on the claude actuator: the `/model`-echo contamination. Not re-reachable while 93a stands. |
| 100 | ✅ **FIXED on v4** — `_confirm_tui_dialog` polls `capture-pane` for the dialog; no fixed sleeps on the decision path. Still open on **v3**, and closes there with the same port as row 95. |
