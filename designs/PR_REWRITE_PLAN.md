...
3. **Carries the inputs onto the branch.** None of 0c's documents and none of the verification harness exists on `origin/main`. Wave 0 copies the seven doc paths in 3a, plus `dev-env.sh`, `run-server.sh`, `run-host.sh`, `run-frontend.sh`, and `scripts/probe_routing_api.sh`, from `routing-mvp-v1`. Without this step every stream's required reading and both live barriers are unreachable.
