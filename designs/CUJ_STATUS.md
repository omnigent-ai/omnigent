...
| In-session Subagent routing row (Smart Routing / Default, two-state) | R0, gear → Subagent routing                                 | UI row toggles                                                                                                          | 🟡 not exercised — headless round, no browser; the underlying `PATCH subagent_routing_override` it drives is ✅ (row below) | pre-2245f57d                         |
...
| Check                                                                    | How to run                                                              | Ground-truth signal                                                                                                  | Status                                     | Last verified |
| ------------------------------------------------------------------------ | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ------------- |
| Smart Routing selectable on a bundle agent's gear config                 | R0, pick debby (or polly) in the new-chat dialog, open the gear          | The config modal offers Smart Routing via the brain-harness override; title/labels read correctly                     | 🟡 vitest (`NewChatDialog.test.tsx`, real debby/polly fixtures) — needs user eyeball | 2026-08-03 |
| Config menu renders right with Smart Routing ON                          | Same, toggle Smart Routing on                                            | The Agent Harness row **stays rendered** with the pick readable (was Bug: it unmounted, stranding the choice); locked Permissions below it; gear tooltip mirrors both; flag-off degrade drops a remembered pick quietly | 🟡 vitest, 15 cases (fix `1f99705f`) — needs user eyeball | 2026-08-03 |
| The routed model/harness actually apply on a bundle session's first turn | Create the debby/polly session with routing on, send a prompt (composer) | R1 decision row for the session; the brain (and any spawned children) land the routed arm; `session_overrides` carries the override + `cost_control_mode_override` | ⬜ payload verified by vitest (`harness_override:"auto"` + `cost_control_mode_override:"on"`); live end-to-end still owed | —             |
