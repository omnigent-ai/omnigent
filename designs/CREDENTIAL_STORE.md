...
façade that maps its domain object ⇄ `(secret dict, metadata dict)`. **This base
PR ships only the generic store; the façade and its callers land in the connect
PR (#4235).** The first façade will be:
...
- `GithubConnectionStore` — `provider="github"`; secret = `{access_token,
  ...
  refresh_token_expires_at, scopes}`. It exposes a typed surface (`upsert`,
  `update_tokens`, `get`, `delete`, `list_all`) over this store, so the connect
  routes, the broker endpoint, and the launch path talk to it rather than to the
  generic store — and none of them need to change when a second provider is
  added.
...
This PR adds a single new migration (`add_integration_connections`) that
`CREATE`s `integration_connections` with the generic schema. There is no
...
(`tests/db/test_migration_integration_connections.py`).
