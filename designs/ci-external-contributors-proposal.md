...
| **Ollama** | Cheap: [first-timers gated, returning auto](#empirical-verification--observed-behavior-on-real-fork-prs-june-2026) | No | GitHub UI (undocumented) | [`release.yaml`](https://github.com/ollama/ollama/blob/main/.github/workflows/release.yaml) | PR CI references **no secrets** at all; the only secret-bearing jobs (release signing) live in `release.yaml`, declare `environment: release` (env-scoped secrets), and trigger only on `push:` of `v*` tags — unreachable from any PR. |
