...
describe("filesPanelPreferences", () => {
  ...
  it("round-trips a written preference", () => {
    ...
    expect(readFilesPanelPreferences()).toEqual({ changedOnly: true, sort: "alpha" });
  });
  ...
  it("defaults changedOnly when the stored field has the wrong type", () => {
    ...
    expect(readFilesPanelPreferences()).toEqual({ changedOnly: false, sort: "recent" });
  });
  ...
  it("defaults sort when the stored value is invalid", () => {
    ...
    expect(readFilesPanelPreferences()).toEqual({ changedOnly: true, sort: "recent" });
  });
});
