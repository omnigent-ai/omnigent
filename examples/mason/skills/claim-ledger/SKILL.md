...
---
# Claim ledger
Use `git log --format=%H <old>..<new>` and inspect each commit body and diff. Extract
...
persist queue, source commit, pattern, and timestamp in the record. Reuse a prior
judgement ONLY when its row names evidence and nothing in this pass contradicts
it. Reopen known-wrong, uncertain, or unnamed-evidence rows; D-725 is the worked
case where the recorded layer is wrong.
