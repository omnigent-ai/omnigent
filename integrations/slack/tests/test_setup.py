...
def _view_with_url(url: str) -> dict[str, Any]:
    return {"state": {"values": {URL_BLOCK: {"server_url_input": {"value": url}}}}}
...
def test_select_modal_lists_agents_and_hosts() -> None:
    ...
    assert view["private_metadata"] == "http://omnigent.test"
...
@respx.mock
async def test_url_submit_advances_to_select_modal_with_host_home_workspace(
    tmp_path: Path,
) -> None:
    ...
    respx.get("http://omnigent.test/v1/hosts/h1/filesystem").mock(
        return_value=httpx.Response(
            200,
    ...
    flow = SetupFlow(store=await _store(tmp_path), pool=pool)
    ack = FakeAck()

    try:
        await flow._handle_url_submit(
            ack, {}, _view_with_url("http://omnigent.test"), FakeSetupClient()
        )
    finally:
...
@respx.mock
async def test_url_submit_shows_no_host_guidance_when_no_online_host(tmp_path: Path) -> None:
    respx.get("http://omnigent.test/health").mock(
        return_value=httpx.Response(200, json={"status": "ok"})
    )
    respx.get("http://omnigent.test/v1/agents").mock(
    ...
    assert "omni host --server http://omnigent.test" in body
    assert "/omnigent" in body


@respx.mock
async def test_url_submit_shows_login_in_modal_and_advances_on_approval(tmp_path: Path) -> None:
    ...
    respx.get("http://omnigent.test/v1/me").mock(
    ...
    respx.get("http://omnigent.test/v1/agents").mock(side_effect=_agents)
    respx.get("http://omnigent.test/v1/hosts").mock(
        return_value=httpx.Response(
            200, json={"hosts": [{"host_id": "h1", "name": "H", "status": "online"}]}
        )
    ...
    authorize_route = respx.post("http://omnigent.test/oauth/device/authorize").mock(
        return_value=httpx.Response(
            200,
            json={
                "device_code": "dc",
                "user_code": "ABCD-2345",
                "verification_uri": "http://omnigent.test/oauth/device",
                "verification_uri_complete": (
                    "http://omnigent.test/oauth/device?user_code=ABCD-2345"
                ),
                "expires_in": 600,
                "interval": 0,
            },
    ...
    respx.post("http://omnigent.test/oauth/token").mock(
        return_value=httpx.Response(
    ...
    flow = SetupFlow(store=await _store(tmp_path), pool=pool, auth_manager=auth)
    ack = FakeAck()
    client = FakeSetupClient()
    body = {"team": {"id": "T1"}, "user": {"id": "U1"}, "view": {"id": "V1"}}

    try:
        await flow._handle_url_submit(ack, body, _view_with_url("http://omnigent.test"), client)

        # The modal shows the login link in place (not a DM).
        assert ack.calls[0]["response_action"] == "update"
        waiting = ack.calls[0]["view"]["blocks"][0]["text"]["text"]
        ...
        for _ in range(50):
            if client.updated_views:
                break
            await asyncio.sleep(0.05)
    finally:
        await pool.aclose_all()

    assert client.updated_views, "modal was never advanced after approval"
...
@respx.mock
async def test_url_submit_auth_required_but_login_disabled(tmp_path: Path) -> None:
    """With no auth manager, an auth-enabled server shows a plain error."""
    respx.get("http://omnigent.test/health").mock(
        return_value=httpx.Response(200, json={"status": "ok"})
    )
    respx.get("http://omnigent.test/v1/agents").mock(return_value=httpx.Response(401))
    pool = OmnigentClientPool()
    flow = SetupFlow(store=await _store(tmp_path), pool=pool)  # no auth_manager
    ack = FakeAck()
    client = FakeSetupClient()
    ...
    # Coherent error in the modal — not a "check your DM" promise.
    assert ack.calls[0]["response_action"] == "errors"
    assert "isn't configured" in ack.calls[0]["errors"][URL_BLOCK]
    assert client.posts == []


async def test_unknown_argument_opens_setup_modal(tmp_path: Path) -> None:
    """Any non-`logout` argument opens the setup modal."""
    pool = OmnigentClientPool()
    flow = SetupFlow(store=await _store(tmp_path), pool=pool)
    client = FakeSetupClient()
    command = {
    ...
    assert client.opened_views[0]["view"]["callback_id"] == "omnigent_setup_url"
    assert client.posts == []
...
async def test_logout_revokes_all_and_clears_settings(tmp_path: Path) -> None:
    ...
    await store.upsert_user_config(
        "T1", "U1", UserConfig("http://s", "ag_1", "Helper", "/home/bob", "h1", "H")
    )
    await store.upsert_session(
        ThreadKey("T1", "C1", "100.1"), "conv_1", "t", owner_user_id="U1", server_url="http://s"
    )

    auth = FakeAuth()
    pool = OmnigentClientPool()
    flow = SetupFlow(store=store, pool=pool, auth_manager=auth)  # type: ignore[arg-type]
    client = FakeSetupClient()
...
@respx.mock
async def test_url_submit_reports_unreachable(tmp_path: Path) -> None:
    respx.get("http://omnigent.test/health").mock(return_value=httpx.Response(500))
    pool = OmnigentClientPool()
    flow = SetupFlow(store=await _store(tmp_path), pool=pool)
    ack = FakeAck()
    ...
    assert ack.calls[0]["response_action"] == "errors"
    assert "reach" in ack.calls[0]["errors"][URL_BLOCK]


async def test_url_submit_rejects_bad_scheme(tmp_path: Path) -> None:
    pool = OmnigentClientPool()
    flow = SetupFlow(store=await _store(tmp_path), pool=pool)
    ack = FakeAck()

    try:
        await flow._handle_url_submit(ack, {}, _view_with_url("omnigent.test"), FakeSetupClient())
    finally:
        await pool.aclose_all()
...
async def test_select_submit_persists_config(tmp_path: Path) -> None:
    store = await _store(tmp_path)
    pool = OmnigentClientPool()
    flow = SetupFlow(store=store, pool=pool)
    ack = FakeAck()
    client = FakeSetupClient()

    view = {
        "private_metadata": "http://omnigent.test",
        "state": {
            "values": {
                AGENT_BLOCK: {
                    "agent_select": {
    ...
    assert config.server_url == "http://omnigent.test"
...
def test_no_host_modal_shows_guidance() -> None:
    view = no_host_modal("http://omnigent.test")
    assert view["callback_id"] == CALLBACK_URL_MODAL
    body = view["blocks"][0]["text"]["text"]
    assert body == host_unavailable_text("http://omnigent.test")
    assert "omni host --server http://omnigent.test" in body
...
def test_url_modal_prefills_server_url() -> None:
    view = url_modal(server_url="http://existing.test")
    element = view["blocks"][0]["element"]
    assert element["action_id"] == URL_ACTION
    assert element["initial_value"] == "http://existing.test"

    # Fresh setup leaves the field empty.
    assert "initial_value" not in url_modal()["blocks"][0]["element"]


async def test_config_command_opens_url_modal(tmp_path: Path) -> None:
    pool = OmnigentClientPool()
    flow = SetupFlow(store=await _store(tmp_path), pool=pool)
    ack = FakeAck()
    client = FakeSetupClient()
    ...
    assert view["callback_id"] == CALLBACK_URL_MODAL
    # No existing config → URL field starts empty.
    assert "initial_value" not in view["blocks"][0]["element"]


async def test_config_command_prefills_current_server(tmp_path: Path) -> None:
    store = await _store(tmp_path)
    await store.upsert_user_config(
        "T1",
        "U1",
        UserConfig(
            server_url="http://existing.test",
            agent_id="ag_1",
            agent_name="Helper",
            workspace="/home/me",
        ),
    ...
    try:
        await flow._handle_config_command(
            ack,
            {"trigger_id": "tid-2", "team_id": "T1", "user_id": "U1"},
            client,
        )
    finally:
        await pool.aclose_all()
    ...
    view = client.opened_views[0]["view"]
    assert view["blocks"][0]["element"]["initial_value"] == "http://existing.test"
