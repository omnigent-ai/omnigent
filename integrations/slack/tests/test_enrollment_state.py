...
def test_sign_verify_roundtrip() -> None:
    state = sign_state("T123", "U456", _EMAIL, _SECRET, team_name="Acme", issued_at=1000)
...
def test_verify_rejects_wrong_secret() -> None:
    state = sign_state("T1", "U1", _EMAIL, _SECRET, issued_at=1000)
...
def test_verify_rejects_future_dated() -> None:
    state = sign_state("T1", "U1", _EMAIL, _SECRET, issued_at=5000)
    with pytest.raises(StateError):
