...
Each Discord user who authenticates via the device flow gets a delegated
access + refresh token for their Omnigent server (see
``designs/DEVICE_AUTH.md``). Those are bearer credentials that let this
process act as that user.
...
- :class:`EncryptedTokenStore` — persisted to SQLite, encrypted with a
  ...
  ``OMNIGENT_DISCORD_TOKEN_ENCRYPTION_KEY`` is configured.
...
Both are keyed by ``(user_id, server_url)``. Unlike Slack — where a user id
is scoped to one workspace and must be paired with a ``team_id`` — a Discord
snowflake user id is globally unique, so the guild plays no part in the key
and a user who signs in once is signed in everywhere the bot can see them.
The server url stays in the key so tokens are strictly scoped to the server
that issued them (and separate cleanly if the operator repoints the bot).
"""
...
class TokenStore(Protocol):
    ...
    async def get(self, user_id: str, server_url: str) -> TokenRecord | None: ...

    async def list_for_user(self, user_id: str) -> list[tuple[str, TokenRecord]]: ...
    ...
    async def delete(self, user_id: str, server_url: str) -> None: ...


class EncryptedTokenStore:
    """Fernet-encrypted, SQLite-persisted store for delegated tokens.

    :param path: SQLite file (shared with :class:`SQLiteStore` or its own
        file — only this class touches the ``oauth_tokens`` table).
    ...
    async def get(self, user_id: str, server_url: str) -> TokenRecord | None:
        ...
        try:
            ...
            # Key rotated or DB tampered — treat as no token so the user is
            # prompted to re-authenticate rather than crashing.
            return None
    ...
    async def list_for_user(self, user_id: str) -> list[tuple[str, TokenRecord]]:
        ...
        (wrong key) are skipped — they can't be revoked but are cleared by
        the accompanying delete.
        """
        async with aiosqlite.connect(self._path) as db:
            cursor = await db.execute(
                """
        ...
        *,
        ...
        async with aiosqlite.connect(self._path) as db:
            await db.execute(
                ...
                """,
                (user_id, server_url, access_enc, refresh_enc, now),
            )
            await db.commit()
...
class InMemoryTokenStore:
    ...
    The fallback when no encryption key is configured. Delegated tokens are
    bearer credentials, so writing them to disk in the clear is not
    acceptable; keeping them in memory lets the integration work while
    bounding exposure to the process lifetime. Tokens are lost on restart, so
    users re-authenticate — a deliberate trade rather than disabling the
    integration.
    """

    def __init__(self) -> None:
        self._tokens: dict[tuple[str, str], TokenRecord] = {}

    async def initialize(self) -> None:
        return None

    async def get(self, user_id: str, server_url: str) -> TokenRecord | None:
        return self._tokens.get((user_id, server_url.rstrip("/")))

    async def list_for_user(self, user_id: str) -> list[tuple[str, TokenRecord]]:
        return [
            (server, record) for (user, server), record in self._tokens.items() if user == user_id
        ]
    ...
    async def put(
        ...
        *,
        ...
        self._tokens[(user_id, server_url.rstrip("/"))] = TokenRecord(
    ...
    async def delete(self, user_id: str, server_url: str) -> None:
        self._tokens.pop((user_id, server_url.rstrip("/")), None)
