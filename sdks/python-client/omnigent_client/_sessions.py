...
async def _stream_session_events(
    ...
    async with http.stream(
        ...
        if resp.status_code >= 400:
            ...
            # An unfollowed redirect: OmnigentClient follows redirects, so a
            # 3xx here means the hop could not be chased (no Location header,
            # or a caller-supplied client with redirects disabled). Parsing
