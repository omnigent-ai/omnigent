...
def test_wait_for_claude_prompt_ready_reports_the_blocking_cause(
    ...
    """The timeout names its cause in the message and on the exception.
    ...
    assert "blocked_on=launch-preamble" in message
...
def test_empty_capture_timeout_reports_no_output_cause(
    ...
    assert "blocked_on=no-output" in str(excinfo.value)
