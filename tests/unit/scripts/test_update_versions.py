...
def _load_update_versions() -> Any:
    """Load ``scripts/update_versions.py`` by file path.

    ``scripts/`` is not a package on ``sys.path``, and a regular test
    package of the same basename (``tests/unit/scripts/``) can shadow the
    top-level ``scripts`` name depending on import order. Load it directly
    (mirrors ``test_normalize_uv_lock_registry.py``).
    """
    path = _REPO_ROOT / "scripts" / "update_versions.py"
    spec = importlib.util.spec_from_file_location("update_versions", path)
    assert spec and spec.loader
    module = importlib.util.module_from_spec(spec)
    # Register before exec: update_versions.py defines @dataclass types, and
    # dataclasses resolves field types via ``sys.modules[cls.__module__]``,
    # which is None unless the module is registered under its own name.
