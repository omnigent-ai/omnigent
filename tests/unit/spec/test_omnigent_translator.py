...
# via ``tests.unit.spec.test_omnigent_translator.sample_tool_callable``.
...
def test_spec_with_local_tool_resolves_dotted_path(
    ...
    **What breaks if this fails**: the harness receives a tool
    ...
    basic_spec.local_tools = [
        LocalToolInfo(
            name="sample",
            path="tests.unit.spec.test_omnigent_translator.sample_tool_callable",
            language="python",
        ),
    ]
    agent_def = agent_spec_to_agent_def(basic_spec)
...
def test_tool_pointing_at_non_callable_rejected(
    ...
    **What breaks if this fails**: the omnigent harness
    ...
    basic_spec.local_tools = [
        LocalToolInfo(
            name="stringy",
            path="tests.unit.spec.test_omnigent_translator.NOT_A_FUNCTION",
            language="python",
        ),
    ]
