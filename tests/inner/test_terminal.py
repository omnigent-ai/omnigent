...
def test_orphan_sweep_preserves_terminals_when_linux_namespace_is_unreadable(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    marker_namespace: str,
) -> None:
    """An unavailable namespace cannot establish that a recorded owner is local."""
    instance_dir = _write_instance_dir(
        tmp_path, "omnigent-terminal-unknownns", _dead_pid(), pid_ns=marker_namespace
    )
    monkeypatch.setattr(terminal_mod, "_terminals_tmp_root", lambda: tmp_path)
    monkeypatch.setattr(terminal_mod, "_tmux_available", lambda: True)
    ...
    monkeypatch.setattr(terminal_mod.os, "readlink", unreadable)

    assert terminal_mod.reap_orphaned_terminals() == 0
    assert instance_dir.exists()


@pytest.mark.parametrize("marker", [b"\xff", b"0\npid_ns=none\n", b"123\npid_ns=\n"])
def test_orphan_sweep_ignores_malformed_owner_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    marker: bytes,
) -> None:
    """A damaged marker neither condemns a terminal nor aborts runner startup."""
    instance_dir = tmp_path / "omnigent-terminal-damaged"
    instance_dir.mkdir()
