...
def test_wait_for_server_uses_fast_poll_before_backoff(
    ...
    class _FakeClient:
        ...
        def __init__(self, **_kwargs: object) -> None:
            ...
            assert _kwargs.get("trust_env") is False
...
def test_wait_for_remote_runner_uses_status_endpoint_and_auth(
    ...
    class _FakeClient:
        ...
        def __init__(self, *, headers: dict[str, str], timeout: float, trust_env: bool) -> None:
            del timeout
            # Remote targets keep httpx's default env proxy handling.
...
def test_wait_for_remote_runner_surfaces_unparseable_proxy_env(
    ...
    parse raises ``InvalidURL`` when the poll's shared client is
    constructed. That failure is deterministic, so the poll must raise the
    actionable ClickException immediately instead of burning the timeout
    ...
    monkeypatch.setattr("omnigent.chat.httpx.Client", _invalid_proxy_env)
