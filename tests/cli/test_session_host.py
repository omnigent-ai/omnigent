...
async def test_auth_commits_fallback_routing_only_after_success(
    monkeypatch: pytest.MonkeyPatch, status_code: int, keyless: bool, asynchronous: bool
) -> None:
    mount = "https://acme.databricks.com/api/2.0/omnigent"
    monkeypatch.setenv("OMNIGENT_REMOTE_AUTH_TOKEN", "test-token")
    set_session_host("conv_1", "host_current")
    auth = _DatabricksTokenAuth(
        server_url=mount,
        session_id="conv_1",
        suppress_slice_key_for_host="host_current" if keyless else None,
    )
    seen: list[str | None] = []

    def respond(request: httpx.Request) -> httpx.Response:
        seen.append(request.headers.get(OMNIGENT_SLICE_KEY_HEADER))
        if len(seen) == 1:
            return httpx.Response(400, json={"error": {"code": "wrong_replica"}})
        return httpx.Response(status_code, json={})

    transport = httpx.MockTransport(respond)
    url = f"{mount}/v1/sessions/conv_1/events"
    if asynchronous:
        async with httpx.AsyncClient(transport=transport, auth=auth) as client:
            response = await client.post(url, json={"type": "retry_session", "data": {}})
    else:
        with httpx.Client(transport=transport, auth=auth) as client:
            response = client.post(url, json={"type": "retry_session", "data": {}})

    assert response.status_code == status_code
    assert seen == ([None, "host_current"] if keyless else ["host_current", None])
    expected_keyless = not keyless if status_code == 200 else keyless
    assert _slice_key(auth, url) == (None if expected_keyless else "host_current")
