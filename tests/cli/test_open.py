...
@pytest.mark.parametrize("permission_level", [1, 2, 3, "4", True])
...
@pytest.mark.parametrize("single_user", [True, False, None])
@pytest.mark.parametrize("include_level", [True, False])
def test_open_unknown_native_permission_requires_single_user_server(
    monkeypatch: pytest.MonkeyPatch, single_user: bool | None, include_level: bool
) -> None:
    snapshot = {
        "host_id": "host_remote",
        "labels": {"omnigent.wrapper": "codex-native-ui"},
    ...
    if include_level:
        snapshot["permission_level"] = None
    calls = _script_http(
        monkeypatch,
        _HostHttpResult(200, snapshot),
        _HostHttpResult(200, {"single_user": single_user}),
        _HostHttpResult(202, {"recovery": "native_terminal_ready"}),
    )
    attach = Mock()
    monkeypatch.setattr(cli_module, "_attach_recovered_open_session", attach)

    result = CliRunner().invoke(cli, ["open", "conv_123", "--server", _BASE_URL])

    assert calls[1]["path"] == "/v1/info"
...
@pytest.mark.parametrize("status_code", [0, 503])
def test_open_unknown_native_permission_survives_server_info_outage(
    monkeypatch: pytest.MonkeyPatch, status_code: int
) -> None:
    _script_http(
        monkeypatch,
        _HostHttpResult(200, {"labels": {"omnigent.wrapper": "codex-native-ui"}}),
        _HostHttpResult(status_code, "temporarily unavailable"),
    )
    ...
    assert (
        cli_module._recover_open_session_while_waiting(
            base_url=_BASE_URL, conversation_id="conv_123", previous=previous
        )
        is previous
    )
...
def test_host_http_json_keeps_routing_preference_after_failed_fallback(
    monkeypatch: pytest.MonkeyPatch, status_code: int, keyless: bool
) -> None:
    headers = {OMNIGENT_SLICE_KEY_HEADER: "host_remote"}
    monkeypatch.setattr("omnigent.chat._remote_headers", Mock(return_value=headers))
    client_headers = _patch_host_http_client(
        monkeypatch,
        [
            _FakeHttpResponse(400, {"error": {"code": "wrong_replica"}}),
    ...
    demotion = (_BASE_URL, "host_remote")
    if keyless:
        cli_module._host_http_keyless_demotions.add(demotion)

    result = cli_module._host_http_json(
        base_url=_BASE_URL, method="GET", path="/v1/sessions/conv_123", host_id="host_remote"
    )

    assert result.status_code == status_code
    assert len(client_headers) == 2
    assert (demotion in cli_module._host_http_keyless_demotions) is keyless
