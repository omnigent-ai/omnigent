...
def stores(
    ...
    host_store.upsert_on_connect(_HOST_SP, "coda-app", "app-sp@example")
...
def test_auth_disabled_allows(
    ...
    assert check_host_access(None, _HOST_SP, HOST_LEVEL_MANAGE, host_perm, host_store, perm)
    assert get_host_permission_level(None, _HOST_SP, host_perm, host_store, perm) == (
        HOST_LEVEL_OWNER
    )
...
def test_owner_allowed_without_grant(
    ...
    assert check_host_access(
        "app-sp@example", _HOST_SP, HOST_LEVEL_MANAGE, host_perm, host_store, perm
    )
    assert (
        get_host_permission_level("app-sp@example", _HOST_SP, host_perm, host_store, perm)
        == HOST_LEVEL_OWNER
    )
...
def test_admin_allowed_without_grant(
    ...
    assert check_host_access(
        "admin@example", _HOST_SP, HOST_LEVEL_MANAGE, host_perm, host_store, perm
    )
    # Display: not "owner" — the admin neither owns nor has a grant here.
    assert (
        get_host_permission_level("admin@example", _HOST_SP, host_perm, host_store, perm) is None
    )

    # With an explicit grant, the admin's displayed level is that grant.
    host_perm.grant("admin@example", _HOST_SP, HOST_LEVEL_USE)
    assert (
...
def test_grant_level_ordering(
    ...
    """
    host_store, host_perm, perm = stores
    host_perm.grant("viewer@example", _HOST_SP, HOST_LEVEL_VIEW)
    host_perm.grant("user@example", _HOST_SP, HOST_LEVEL_USE)

    assert check_host_access(
        "viewer@example", _HOST_SP, HOST_LEVEL_VIEW, host_perm, host_store, perm
    )
    assert not check_host_access(
        "viewer@example", _HOST_SP, HOST_LEVEL_USE, host_perm, host_store, perm
    )

    assert check_host_access("user@example", _HOST_SP, HOST_LEVEL_USE, host_perm, host_store, perm)
    assert not check_host_access(
        "user@example", _HOST_SP, HOST_LEVEL_MANAGE, host_perm, host_store, perm
    )

    assert get_host_permission_level("viewer@example", _HOST_SP, host_perm, host_store, perm) == 1
    assert get_host_permission_level("user@example", _HOST_SP, host_perm, host_store, perm) == 2
...
def test_no_grant_denied(
    ...
    host_store, host_perm, perm = stores
    assert not check_host_access(
        "stranger@example", _HOST_SP, HOST_LEVEL_VIEW, host_perm, host_store, perm
    )
    assert (
        get_host_permission_level("stranger@example", _HOST_SP, host_perm, host_store, perm)
        is None
    )
