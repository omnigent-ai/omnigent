...
def test_from_env_disabled_without_enc_key(monkeypatch: pytest.MonkeyPatch) -> None:
    # Client id/secret and a resolvable redirect are present, but the
    # dedicated token-encryption key is not — the feature fails closed.
    _clear_env(monkeypatch)
    monkeypatch.setenv("OMNIGENT_GITHUB_APP_CLIENT_ID", "Iv1abc")
    monkeypatch.setenv("OMNIGENT_GITHUB_APP_CLIENT_SECRET", "shh")
...
def test_from_env_derives_redirect_from_domain(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # The token-encryption key comes from the dedicated env var only.
