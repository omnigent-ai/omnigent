...
class _FakeCipher:
    ...
    :class:`~omnigent.server.secretbox.KmsSecretCipher`, without AWS. The
...
def _store(db_uri: str) -> IntegrationCredentialStore:
    return IntegrationCredentialStore(db_uri, _FakeCipher())
...
def test_secret_never_stored_in_plaintext(db_uri: str) -> None:
    ...
    # column, only Fernet ciphertext.
    store = _store(db_uri)
    store.upsert("dave", "github", secret={"access_token": "ghu_plaintext_leak"}, metadata={})
    engine = get_or_create_engine(db_uri)
    with engine.connect() as conn:
        stored = conn.execute(
            sa.text("SELECT secret_enc FROM integration_connections")
        ).scalar_one()
...
def test_wrong_key_decrypts_to_none(db_uri: str) -> None:
    ...
    other = IntegrationCredentialStore(db_uri, _FakeCipher("different-key"))
