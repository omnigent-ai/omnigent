...
class FiredRecord:
    ...
    async def on_fire(self, scheduled_task_id: str) -> None:
        self.ids.append(scheduled_task_id)
...
async def test_fire_invokes_on_fire_callback() -> None:
    scheduler, _clock, seam, fired = _make([_task("a")])
    await scheduler.start()
...
async def test_overlapping_fire_is_skipped() -> None:
    ...
    async def slow_on_fire(task_id: str) -> None:
    ...
    assert calls == ["a"]  # not fired twice
