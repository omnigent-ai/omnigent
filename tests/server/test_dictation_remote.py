...
def test_availability_with_remote_url(monkeypatch: pytest.MonkeyPatch) -> None:
    """A configured worker URL advertises dictation without local models."""
...
def test_get_engine_prefers_remote(monkeypatch: pytest.MonkeyPatch) -> None:
    """REMOTE_URL selects the relay engine (fake env cleared first)."""
    monkeypatch.delenv(dictation.ENGINE_ENV, raising=False)
    monkeypatch.setenv(dictation.REMOTE_URL_ENV, "ws://example:8100/v1/dictation/stream")
    monkeypatch.setattr(dictation, "_engine", None)
