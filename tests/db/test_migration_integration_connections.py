"""Tests for the integration_connections migration (ga1b2c3d4e5f).
...
def test_upgrade_creates_table_downgrade_drops_it(tmp_path: Path) -> None:
    uri = f"sqlite:///{tmp_path / 'integration_connections.db'}"
    ...
    pk = set(inspector.get_pk_constraint("integration_connections")["constrained_columns"])
    assert pk == {"workspace_id", "user_id", "provider", "account_id"}

    _downgrade(uri, engine, "d5e9f1a2b3c4")
    assert "integration_connections" not in sa.inspect(engine).get_table_names()
