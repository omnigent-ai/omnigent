...
async def test_a_codex_tui_recreated_on_its_live_app_server_keeps_the_turn(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A lost codex TUI comes back on the app-server that still runs its turn.

    The TUI's exit keeps the turn's status and every sidecar, so the ensure
    re-creates the TUI on the same app-server and forwarder instead of
    restarting the turn. The new pane owns the session again: the sweep no
    longer offers it, the turn's status holds until its relayed idle, and the
    reap then releases the reused sidecars with the pane.
    """
    ...
    rig, clock, sidecars = await _rig(tmp_path, monkeypatch, "codex")
    report_harness_state(rig, monkeypatch, tmp_path, "active")
    server = sidecars.codex_app_server
    assert server is not None
    # What the ensure checks before reusing an app-server: a live process on
    # the socket and home the bridge state names.
    server.proc = SimpleNamespace(returncode=None)  # type: ignore[attr-defined]
    server.listen_url = "ws://127.0.0.1:1"  # type: ignore[attr-defined]
    server.codex_home = tmp_path  # type: ignore[attr-defined]
    server.config_overrides = []  # type: ignore[attr-defined]
    server.codex_cli_version = None  # type: ignore[attr-defined]
    server.codex_path = "codex"  # type: ignore[attr-defined]
    server.env = {}  # type: ignore[attr-defined]

    class _LaunchConfig(SimpleNamespace):
        # Only the cold path, which starts a new app-server, reads more.
        def __getattr__(self, name: str) -> object:
            raise AssertionError(f"the ensure restarted the app-server (read {name})")

    async def _launch_config(**_kwargs: object) -> SimpleNamespace:
    ...
    monkeypatch.setattr(orchestration, "_codex_native_launch_config", _launch_config)
    recovered = make_test_terminal_instance("codex", "main", tmp_path / "recovered")
    recovered.start_idle_watcher_thread = lambda *_a, **_k: None  # type: ignore[method-assign]
    recovered.pane_pid_sync = lambda: None  # type: ignore[method-assign]

    async def _close() -> None:
        recovered.running = False

    recovered.close = _close  # type: ignore[method-assign]
    launched: list[list[str]] = []

    async def _launch(*, conversation_id: str, terminal_name: str, **kwargs: object) -> object:
        launched.append(list(kwargs["spec"].args))  # type: ignore[attr-defined]
        panes = rig.terminal_registry._by_conversation.setdefault(conversation_id, {})
        panes[(terminal_name, "main")] = recovered
        return recovered

    monkeypatch.setattr(rig.terminal_registry, "launch", _launch)
    rig.book.record(rig.conv_id, "running", source=StatusSource.RELAY)
    try:
        ...
        assert [row.kind for row in _runtime_rows(rig)] == [RUNTIME_KIND]

        await orchestration._auto_create_codex_terminal(
            rig.conv_id, rig.resources, lambda _sid, _event: None
        )
        assert rig.terminal_registry.get(rig.conv_id, "codex", "main") is recovered
        assert len(launched) == 1 and "thread_1" in launched[0]
        assert orchestration._AUTO_CODEX_APP_SERVERS[rig.conv_id] is server
        assert orchestration._AUTO_FORWARDER_TASKS[rig.conv_id] is sidecars.forwarder
        assert sidecars.intact(rig.app)
        record = rig.book.current(rig.conv_id)
        assert record is not None and record.status == "running"
        assert _runtime_rows(rig) == []
        ...
        for task in list(orchestration._TERMINAL_INTERACTIVE_TASKS):
            task.cancel()
        sidecars.discard()
