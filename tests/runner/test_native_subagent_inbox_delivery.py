...
@pytest.mark.asyncio
async def test_nested_subagent_parent_without_inbox_receives_completion(
    _clean_subagent_registry: None,
) -> None:
    ...
    assert http == 204
    assert len(items) == 1
    assert items[0]["task_id"] == CHILD_SESSION_ID
    assert items[0]["status"] == "completed"
...
@pytest.mark.asyncio
async def test_cold_parent_completion_is_preserved_when_inbox_recovery_runs(
    _clean_subagent_registry: None,
) -> None:
    """Later recovery keeps the inbox seeded by a completion without duplicating it."""
    child_body = _child_snapshot(sub_agent_name="reviewer", parent_session_id=PARENT_SESSION_ID)
    pm = _FakeProcessManager(_ScriptedHarnessClient([]))
    ...
    async with _runner_client(app) as client:
        ...
        original_inbox = runner_app._session_inboxes_ref[PARENT_SESSION_ID]
        assert original_inbox.qsize() == 1

        await app.state.recover_undrained_subagent_results(PARENT_SESSION_ID)
        await app.state.recover_undrained_subagent_results(PARENT_SESSION_ID)

    inbox = runner_app._session_inboxes_ref[PARENT_SESSION_ID]
    assert inbox is original_inbox
