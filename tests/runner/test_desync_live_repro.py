...
class _ChainProcessManager:
    ...
    def __init__(self) -> None:
        ...
        # In-flight tracking the runner drives on response.created / stream end.
        # Recorded so tests can assert a continuation actually marked/cleared it
        # (its ABSENCE previously let the continuation AttributeError silently).
...
async def test_get_client_signals_resync_on_model_and_agent_switch(
    ...
    # Seed a live entry on model-A that is MID-RESPONSE — only then does a
    # respawn strand a turn, so only then must the hook fire.
    pm._entries[conv] = _fake_entry(harness, "model-A")
    pm._in_flight_response_ids[conv] = "resp_live_1"

    # 1) Model switch A -> B respawns and signals, carrying the replaced id.
    ...
    # 3) Agent (harness) switch respawns and signals (still mid-response).
    signals.clear()
    ...
    # 4) A model switch with NO in-flight response (the between-turns / new-turn's
    #    own-setup case) does NOT signal — respawning strands no turn, and firing
    #    would cancel the very turn being started.
    signals.clear()
    ...
    # 5) Crash respawn (dead subprocess) does NOT signal — the orphan/teardown
...
async def test_get_client_isolates_respawn_hook_failure(
    ...
    model_key = _model_env_key(harness)
    pm._entries[conv] = _fake_entry(harness, "model-A")
    # Mid-response, so the model switch below actually fires the (raising) hook.
...
async def test_respawn_adapter_gates_on_active_turn() -> None:
    ...
    A respawn between turns (no turn running) must be a clean no-op. A respawn
    whose replaced response id does NOT match the currently-bound turn (the
    wedged turn ended and a fresh one took the slot) must also be a no-op —
    cancelling there would clobber a healthy turn. Only a respawn that races the
    SAME turn whose subprocess was replaced drives the real
    ``_resync_turn_state``.
    """
    ...
    # Active turn but the replaced id does NOT match the bound turn → no-op (a
    # newer turn owns the slot; the identity gate must protect it).
    app.state.active_turns[conv] = None
    ...
    # Active stream-mode turn whose response id MATCHES the replaced one → real
    # resync clears the gate + publishes once.
    app.state.active_turns[conv] = None
...
async def _run_respawn_chain(
    ...
    consume_task, pm)``; the caller awaits the cancelled turn's teardown and can
    assert the continuation drove the stub pm's in-flight bookkeeping.
    """
    ...
    await _spin_until(lambda: _wedged_dispatch_call_id(adapter) is not None)
    app.state.active_turns[conv] = None
    # The bound turn's response id, matched by the respawn hook's identity gate.
    app.state.live_response_id[conv] = "resp_wedged"
...
async def test_model_switch_mid_turn_orphan_burst_next_turn_recovers(
    ...
    * a SUBSEQUENT real harness turn rebinds a live ctx and dispatches a tool to
    ...
    with caplog.at_level(logging.ERROR, logger=_ADAPTER_LOGGER):
        ...
        # The BUFFERED continuation actually RAN to completion — not silently
        # dropped on an AttributeError (the stub pm previously lacked
        # ``clear_in_flight``, which the continuation's stream teardown calls).
        # Its ``_run_turn_bg`` clears the desynced flag at start and drains the
        # buffer; its stream end calls ``clear_in_flight``. Assert all three so
        # this test genuinely proves the continuation completed.
        await _spin_until(
            lambda: (
