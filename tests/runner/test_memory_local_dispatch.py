...
to memory_retain falls through to the harness, which has no such tool, and
...
def _spec(config: dict[str, str], name: str = "memory_retain") -> SimpleNamespace:
...
def test_memory_tool_set_is_exactly_the_three() -> None:
    assert set(_MEMORY_TOOLS) == {"memory_retain", "memory_recall", "memory_reflect"}


@pytest.mark.parametrize("name", ["memory_retain", "memory_recall", "memory_reflect"])
...
def test_recall_dispatch_honors_config_bank_id() -> None:
    ...
    spec = _spec({"api_key": "hsk_test", "bank_id": "remy-ui-test"}, name="memory_recall")
    with patch("hindsight_client.Hindsight", return_value=client):
        result = asyncio.run(
            _execute_memory_tool(
                {"query": "favorite db"},
