...
def test_unavailable_when_owner_not_connected(monkeypatch: pytest.MonkeyPatch) -> None:
    _connect(monkeypatch, token=None)  # coords present, broker says not connected
    assert github_mcp_available() is False
    assert github_mcp_token() is None
    assert github_mcp_server_config() is None
