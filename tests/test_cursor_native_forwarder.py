...
class TestPostExternalSessionRotated:
    ...
    @pytest.mark.asyncio
    async def test_rejection_is_settled_and_logged(self, caplog: pytest.LogCaptureFixture) -> None:
...
class TestForwardLoopClearRotation:
    ...
    async def test_new_chat_rotates_mirror_and_resume_target(
        ...
        async def body(bridge_dir: Path) -> None:
            await self._wait_for(
                lambda: (
                    fwd._read_state(bridge_dir).store_path == str(first)
    ...
    async def test_rotation_yields_to_a_live_sibling_claim(
        ...
        async def body(bridge_dir: Path) -> None:
            ...
            sibling.mkdir()
            second_store = (
                chats_root / hashlib.md5(self._WS.encode()).hexdigest() / _CHAT_ID_2 / "store.db"
            )
            fwd._write_state(
                sibling,
                fwd._ForwardState(
