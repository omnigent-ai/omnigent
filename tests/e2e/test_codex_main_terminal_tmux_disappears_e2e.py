...
A user runs a Codex (``codex-native``) session, whose Codex TUI the runner
auto-launches into the session-scoped ``codex:main`` managed tmux terminal.
Mid-session the tmux server backing that terminal goes away. The runner's
threaded idle watcher — armed on exactly this terminal in
``omnigent/runner/resource_registry.py`` — probes the pane every
...
the terminal gone: it logs ``tmux unavailable after 3 consecutive probes for
terminal codex:main`` at ERROR, flips ``instance.running`` to ``False`` and
fires ``on_exit``. In production ``on_exit`` routes through
...
3. Inject the fault: the tmux server backing ``codex:main`` disappears
   (``kill-server`` — the deterministic stand-in for whatever removes it in
   production: an ungraceful runner teardown + startup orphan sweep, OOM, a
   /tmp reaper, a crash).
4. Observe the outcome: within the watcher's probe budget it logs the exact
...
async def test_codex_main_terminal_torn_down_when_tmux_server_disappears(
    ...
    The threaded idle watcher detects the
    ...
    try:
        ...
        with caplog.at_level(logging.ERROR, logger="omnigent.inner.terminal"):
            # THE FAULT: the tmux server backing codex:main disappears.
            subprocess.run(
