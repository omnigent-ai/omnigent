"""Cold exact-model launches bypass discovery while the runner stays responsive."""
...
def _create_session(
    ...
    # Binding immediately starts the terminal, after its selection is persisted.
    patch = httpx.patch(
        f"{base_url}/v1/sessions/{session_id}",
...
@pytest.mark.parametrize("source", ["spec", "picker"])
def test_codex_native_startup_does_not_block_runner_loop(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as spec_tmp:
            spec_path = _materialize_codex_agent_spec(
                Path(spec_tmp), model=ExactCodexModel(_PINNED_MODEL) if source == "spec" else None
            )
            codex_yaml = spec_path.read_text(encoding="utf-8")
        if source == "spec":
            assert "model_resolution: exact" in codex_yaml
        ...
        a_sid = _create_session(
            ...
            selection=(
                {"model_override": "gpt-picker", "model_override_id": _PINNED_MODEL}
                if source == "picker"
                else None
            ),
        )
