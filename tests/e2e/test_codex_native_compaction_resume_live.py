"""Real Codex-native compaction + cold-resume canary (explicit opt-in).

Prerequisites: an idle developer machine, ``codex`` installed and logged in
under the real ``$HOME``, ``tmux``, and a valid Databricks CLI profile. CI must
never set the opt-in variable below. Run exactly:

    OMNIGENT_E2E_CODEX_NATIVE_COMPACTION_RESUME=1 \
    uv run --frozen --group test pytest \
      tests/e2e/test_codex_native_compaction_resume_live.py \
      --profile oss \
      --llm-api-key "$(databricks auth token -p oss |
        python -c 'import json,sys; print(json.load(sys.stdin)["access_token"])')" \
      -vv -s

``OMNIGENT_CODEX_NATIVE_CANARY_SESSIONS`` controls the number of independent
sessions (default/minimum 3). Cases alternate local rollout reuse and forced
server reconstruction. If an installed Codex build refuses ``/compact`` at
...
_OPT_IN = "OMNIGENT_E2E_CODEX_NATIVE_COMPACTION_RESUME"
_COUNT = "OMNIGENT_CODEX_NATIVE_CANARY_SESSIONS"

pytestmark = pytest.mark.skipif(
    os.environ.get(_OPT_IN) != "1" or shutil.which("codex") is None,
    reason=(
        "real Codex compaction/resume canary requires authenticated `codex`; "
...
def test_codex_native_compaction_cold_resume_live(
    session_index: int,
    mode: ResumeMode,
    ...
    """Compact a distinct real Codex conversation, cold-resume, and recall."""
