...
The Hindsight client (``hindsight-client``, the optional ``memory`` extra,
...
from omnigent.tools.builtins import memory as memory_mod
...
def _clear_created_banks() -> None:
    ...
    memory_mod._CREATED_BANKS.clear()
    yield
    memory_mod._CREATED_BANKS.clear()
...
def test_registry_returns_the_three_tools() -> None:
    assert isinstance(get_builtin_tool("memory_retain", _cfg()), MemoryRetainTool)
    assert isinstance(get_builtin_tool("memory_recall", _cfg()), MemoryRecallTool)
    assert isinstance(get_builtin_tool("memory_reflect", _cfg()), MemoryReflectTool)


def test_names_and_descriptions() -> None:
    assert MemoryRetainTool.name() == "memory_retain"
    assert MemoryRecallTool.name() == "memory_recall"
    assert MemoryReflectTool.name() == "memory_reflect"
    # description() must work without instantiation (used by tool discovery).
    assert "memory" in MemoryRecallTool.description().lower()


@pytest.mark.parametrize(
    ("cls", "param"),
    [
        (MemoryRetainTool, "content"),
        (MemoryRecallTool, "query"),
        (MemoryReflectTool, "query"),
    ],
)
...
def test_retain_stores_and_creates_bank(tool_ctx: ToolContext) -> None:
    client = _mock_client()
    tool = MemoryRetainTool(_cfg(bank_id="alice"))
...
def test_retain_bank_defaults_to_agent_id(tool_ctx: ToolContext) -> None:
    client = _mock_client()
    tool = MemoryRetainTool(_cfg())  # no bank_id
    with patch("hindsight_client.Hindsight", return_value=client):
...
def test_retain_with_tags(tool_ctx: ToolContext) -> None:
    client = _mock_client()
    tool = MemoryRetainTool(_cfg(bank_id="b", tags="env:prod, team:core"))
    with patch("hindsight_client.Hindsight", return_value=client):
        tool.invoke(json.dumps({"content": "x"}), tool_ctx)
...
def test_recall_returns_bullet_list(tool_ctx: ToolContext) -> None:
    ...
    tool = MemoryRecallTool(_cfg(bank_id="b"))
    with patch("hindsight_client.Hindsight", return_value=client):
...
def test_reflect_returns_answer(tool_ctx: ToolContext) -> None:
    ...
    tool = MemoryReflectTool(_cfg(bank_id="b"))
    with patch("hindsight_client.Hindsight", return_value=client):
...
def test_missing_api_key_returns_error(tool_ctx: ToolContext) -> None:
    tool = MemoryRecallTool({})  # no api_key
    result = tool.invoke(json.dumps({"query": "q"}), tool_ctx)
...
def test_client_exception_is_caught(tool_ctx: ToolContext) -> None:
    ...
    with patch("hindsight_client.Hindsight", return_value=client):
        result = tool.invoke(json.dumps({"query": "q"}), tool_ctx)
    assert result.startswith("Memory recall failed:")
