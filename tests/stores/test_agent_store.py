...
def test_session_scoped_agent_resolves_to_oldest_root_on_reuse(
    agent_store: SqlAlchemyAgentStore,
    conversation_store: SqlAlchemyConversationStore,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Reuse across roots resolves to the ORIGINAL (oldest) owning session.

    Binding an existing session-scoped agent into a second top-level session
    (``POST /v1/sessions`` with an ``agent_id``) makes two roots reference the
    same agent. The owning-session lookup must return the oldest — the original
    session the agent was created with — not the reuser's, so a shared user
    cannot reuse a legacy (unowned) agent and have the owner check resolve to
    their own session.
    """
    import omnigent.stores.conversation_store.sqlalchemy_store as cs

    ticks = iter(range(1_000, 9_999))
    monkeypatch.setattr(cs, "now_epoch", lambda: next(ticks))

    created = conversation_store.create_session_with_agent(
        agent_id="a00000000000000000000000000000ab",
        agent_name="reused-agent",
        agent_bundle_location="ag_reused/bundle",
        agent_description=None,
        title="original",
    )
    original_root = created.conversation.id

    # Reuse: a newer, independent top-level session bound to the same agent_id.
    reuse = conversation_store.create_conversation(agent_id=created.agent.id)
    assert reuse.id != original_root

    fetched = agent_store.get(created.agent.id)
    assert fetched is not None
    assert fetched.session_id == original_root
