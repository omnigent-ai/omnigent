...
def test_codex_goal_mode_processes_first_message_with_untrusted_hooks(
    ...
    expect(page.get_by_test_id("composer-goal-mode")).to_have_accessible_name(
        f"Goal paused: {objective}"
    )
    ...
    expect(page.get_by_test_id("composer-goal-mode")).to_have_accessible_name(
        f"Goal active: {objective}", timeout=30_000
    )
