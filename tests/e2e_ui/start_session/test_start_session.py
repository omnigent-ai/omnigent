...
def test_start_session_select_claude_profile(seeded_session: tuple[str, str]) -> None:
    ...
    Issue #503: a claude-sdk agent (Polly) with operator-configured
    ``claude_profiles`` exposes a "Claude account" select in its run-config
    modal. Selecting the ``personal`` profile and saving must reach
    ``POST /v1/sessions`` as ``claude_profile: "personal"`` — mocked end to
    end, no live LLM / CLI invocation.
    """
    base_url, session_id = seeded_session
...
async def _drive_claude_profile(base_url: str, session_id: str) -> None:
    async with async_playwright() as pw:
        ...
        try:
            ...
            # Polly's harness is claude-sdk, so its run-config modal shows
            # the "Claude account" select. Open it from the composer gear.
            await _open_entry_config(page, "ag_polly_e2e")
            await expect(
                page.get_by_test_id("new-chat-landing-config-claude-profile")
            ).to_be_visible()
            # Pick "Personal" and commit the modal draft with Save.
            await _pick_config_select(page, "new-chat-landing-config-claude-profile", "Personal")
            await _save_config(page)
