"""Run the guarded provider browser test without loading unrelated conftests."""
...
def main() -> None:
    ...
    source = Path(__file__).with_name("test_provider_settings.py")
    spec = importlib.util.spec_from_file_location("provider_browser_test", source)
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                module.test_settings_persist_on_selected_host_and_route_new_sessions(page, runtime)
                page.screenshot(path=str(args.recordings / "passed.png"))
            finally:
                context.close()
                browser.close()
