"""Browser e2e for session search (⌘⇧F, or the sidebar's Search button).

The sidebar's "Search" button opens session search, whose input debounces
keystrokes (~300 ms) and forwards the query to the server as
...
query lists the session under the "Sessions" group; a non-matching one
lists nothing and the overlay falls to its "No sessions found" empty
state.

This drives the full chain the ``useConversations`` unit test can't: the
sidebar button → the overlay → the debounce → the ``?search_query=``
...
The ⌘K command palette is a separate surface and lists no sessions; see
...
def test_search_lists_matching_sessions_in_palette(
    ...
    """Session search lists a session matching the query, empties otherwise.

    Sets a unique title on the seeded session, opens session search from
    the sidebar's Search button, then asserts the round-trip both ways:
    ...
    # Open session search from the sidebar's Search button (the pointer-driven
    # equivalent of ⌘⇧F, now that the inline filter box is gone).
    search_button = page.get_by_test_id("sidebar-search-button")
    expect(search_button).to_be_visible(timeout=30_000)
    search_button.click()

    # Scope all result assertions to the overlay dialog: the session's title
    ...
    # A query that matches nothing empties the list and shows the sessions-only
    # empty state (debounce + server round trip resolve within the default
    # expect timeout).
    no_match = f"zzz-no-match-{uuid.uuid4().hex[:12]}"
    palette_input.fill(no_match)
    expect(dialog.get_by_text(title)).to_have_count(0)
    expect(dialog.get_by_text("No sessions found")).to_be_visible()

    # A query matching the title lists the session — proving search runs
    # server-side on the title, not just filters a static page.
