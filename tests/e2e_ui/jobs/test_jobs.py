"""E2E: the Jobs/Workflows journey — create a flow, save it, and run it.

The Jobs page (``/jobs``) lists saved workflows; "Create flow" mints a job and
opens the flow builder (``/jobs/flow/:id``). In the builder the user adds nodes,
picks the agent to run as, saves, and hits "Run now" — which renders the flow's
narrative, creates an agent session seeded with it, and navigates to that
session (``/c/:id``).
...
create → builder → pick agent → run → land on the session. ``live_server`` runs
``examples/hello_world.yaml``, so the agent picker offers the "Hello_world"
agent.
"""
...
def test_create_flow_pick_agent_and_run(page: Page, live_server: str) -> None:
    """Create a job, bind an agent, and run it — landing on the new session.
    ...
    # We land in the flow builder on the new job.
    expect(page).to_have_url(re.compile(r"/jobs/flow/job_[0-9a-f]+"), timeout=30_000)

    # The builder seeds an example chart, so there is already content to run.
    ...
    # Run now persists the latest graph, then runs it. The React Flow canvas
    # renders an overlay that swallows a positional click, so dispatch the
    # click via the DOM to reliably reach the React handler.
    run_button = page.get_by_test_id("job-run-button")
    expect(run_button).to_be_enabled(timeout=30_000)
    run_button.evaluate("el => el.click()")
...
def test_jobs_list_shows_created_job(page: Page, live_server: str) -> None:
    """A created job appears back on the Jobs list with a Run action.

    Guards the list round-trip through the API: after creating a flow and
    navigating back, the job row is present and exposes its inline "Run job
    now" control.
    """
    base_url = live_server
    page.goto(f"{base_url}/jobs")
    ...
    expect(rows.first.get_by_role("button", name="Run job now")).to_be_visible()
