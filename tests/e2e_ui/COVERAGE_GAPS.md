...
| ✅ | **Sidebar toggle hotkeys** (⌘⌥[ / ⌘B / legacy ⌘⌥]) | `hooks/useSidebarToggleHotkeys.ts`, `shell/AppShell.tsx` | `sessions/test_sidebar_toggle_hotkeys.py` — the window-level chords collapse/reopen Conversations and Workspace from a focused composer, Monaco editor, and xterm terminal; the legacy Workspace chord remains covered. Physical-`e.code` matching + modifier gating stay unit-covered by `useSidebarToggleHotkeys.test.tsx`. |
