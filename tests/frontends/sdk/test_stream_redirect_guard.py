...
@pytest.mark.asyncio
async def test_responses_stream_on_redirect_raises_instead_of_completing_empty() -> None:
    """A 302 on the responses stream POST raises ``OmnigentError`` too.

    Fails without the fix: the redirect yields no SSE events, the tool loop
    sees no pending calls and breaks, and ``stream()`` finishes with zero
    events and no error — the same silent empty stream on the second
    stream-open path.
    """
    seen = 0
