...
def _point_codex_auth_check_at(
    ...
    *,
    ...
    ``config_toml`` writes Codex's ``config.toml`` next to ``auth.json`` (the
    real layout: both live in ``CODEX_HOME``); omitted means no config file, so
    only ``auth.json`` can carry a credential.
    """
    if launch is None:
...
def test_codex_auth_unavailable_reason_resolver_failure_skips_config_provider(
    ...
    """A resolver blow-up gates on ``auth.json`` alone, not the config provider.

    Without a resolved launch there is no way to know whether it would pin
    ``model_provider`` over the config's selection, so the unknown case keeps the
    conservative ``auth.json`` answer.
    """
    auth_path = tmp_path / "codex-home" / "auth.json"  # never created
    _point_codex_auth_check_at(
        monkeypatch, auth_path, binary_present=True, config_toml=_ENV_KEY_CONFIG_TOML
