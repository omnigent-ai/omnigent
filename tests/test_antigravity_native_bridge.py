...
# ---------------------------------------------------------------------------
# forwarded_step_index (Finding 1: durable restart/resume dedup cursor)
# ---------------------------------------------------------------------------


def test_forwarded_step_index_roundtrips(bridge_dir: Path) -> None:
    """
    The forwarded_step_index resume cursor roundtrips verbatim.

    The forwarder persists the highest mirrored agy ``step_index`` here so a
    restart/resume re-POSTs only steps beyond it. A dropped value would make a
    restart re-mirror the whole transcript (the bug Finding 1 fixes).
    """
    state = AntigravityNativeBridgeState(
        session_id="conv_abc",
        conversation_id="agy_conv",
    ...
    read_back = read_bridge_state(bridge_dir)
    assert read_back is not None
    assert read_back.forwarded_step_index == 14


def test_forwarded_step_index_defaults_none(bridge_dir: Path) -> None:
    """
    A state written without a cursor reads back with ``forwarded_step_index``
    ``None`` (nothing mirrored yet — a fresh launch).
    """
    write_bridge_state(
        bridge_dir,
        AntigravityNativeBridgeState(session_id="conv_abc", conversation_id="agy_conv"),
    ...
    read_back = read_bridge_state(bridge_dir)
    assert read_back is not None
    assert read_back.forwarded_step_index is None


def test_read_bridge_state_omits_legacy_state_without_cursor(bridge_dir: Path) -> None:
    """
    A legacy state.json with no ``forwarded_step_index`` key parses as ``None``.

    Guards forward-compat: a state file written before the cursor existed must
    read back with the cursor defaulted to ``None`` (mirror from the top), not
    crash.
    """
    (bridge_dir / "state.json").write_text(
        json.dumps({"session_id": "conv_abc", "conversation_id": "agy_conv"}) + "\n",
        encoding="utf-8",
    )
    state = read_bridge_state(bridge_dir)
    assert state is not None
...
@pytest.mark.parametrize("bad_value", [True, False, -1, "5", 1.5])
def test_read_bridge_state_rejects_bad_cursor_types(bridge_dir: Path, bad_value: object) -> None:
    """
    A non-int / bool / negative ``forwarded_step_index`` reads back as ``None``.

    ``bool`` is an ``int`` subclass, so a stray ``true`` must not be read as a
    cursor of 1; a negative value must not be honored either. Defensive against
    a hand-edited or corrupt state file silently suppressing real steps.
    """
    (bridge_dir / "state.json").write_text(
        json.dumps(
            {
                "session_id": "conv_abc",
                "conversation_id": "agy_conv",
                "forwarded_step_index": bad_value,
            }
        )
        + "\n",
        encoding="utf-8",
...
def test_update_forwarded_step_index_sets_value(bridge_dir: Path) -> None:
    """update_forwarded_step_index persists the cursor onto existing state."""
    write_bridge_state(
        bridge_dir,
        AntigravityNativeBridgeState(session_id="conv_abc", conversation_id="agy_conv"),
    )
    update_forwarded_step_index(bridge_dir, 7)
    state = read_bridge_state(bridge_dir)
    assert state is not None
    assert state.forwarded_step_index == 7


def test_update_forwarded_step_index_is_monotonic(bridge_dir: Path) -> None:
    """
    A lower cursor than the stored one is ignored (never rewinds).

    A stale or out-of-order persist must not move the cursor backward, or a
    restart would re-post steps between the lower and higher values.
    """
    write_bridge_state(
        bridge_dir,
        AntigravityNativeBridgeState(
            session_id="conv_abc", conversation_id="agy_conv", forwarded_step_index=10
        ),
    )
    update_forwarded_step_index(bridge_dir, 4)  # lower → ignored
    state = read_bridge_state(bridge_dir)
    assert state is not None
    assert state.forwarded_step_index == 10
    update_forwarded_step_index(bridge_dir, 12)  # higher → advances
    state2 = read_bridge_state(bridge_dir)
    assert state2 is not None
    assert state2.forwarded_step_index == 12


def test_update_forwarded_step_index_noop_when_no_state(bridge_dir: Path) -> None:
...
def test_update_conversation_id_resets_cursor_on_id_change(bridge_dir: Path) -> None:
    """
    A genuinely new conversation id resets the resume cursor to ``None``.

    The cursor is per-conversation: when agy starts a different conversation the
    prior conversation's cursor must not suppress the new transcript's steps.
    """
    write_bridge_state(
        bridge_dir,
        AntigravityNativeBridgeState(
...
def test_update_conversation_id_preserves_cursor_on_same_id(bridge_dir: Path) -> None:
    """
    Re-persisting the *same* conversation id keeps the resume cursor.

    On a forwarder restart that rediscovers the same id, the idempotent
    re-persist must preserve the cursor so already-mirrored steps stay skipped.
    """
    write_bridge_state(
        bridge_dir,
        AntigravityNativeBridgeState(
            session_id="conv_abc",
            conversation_id="agy_conv_same",
            forwarded_step_index=20,
        ),
    )
    update_conversation_id(bridge_dir, "agy_conv_same", active_turn_id="turn_x")
    state = read_bridge_state(bridge_dir)
    assert state is not None
    assert state.conversation_id == "agy_conv_same"
    assert state.active_turn_id == "turn_x"
    assert state.forwarded_step_index == 20
