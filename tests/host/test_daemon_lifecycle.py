...
def test_daemon_owner_is_live_flock_oracle(
    ...
    def rec(*, pid: int, started_at: int, uses_flock: bool) -> cli._HostDaemonRecord:
    ...
    # Held lock → alive, even if the PID looks dead.
    lock = DaemonLifecycleLock.for_target(target, base_dir=tmp_path, pid=999)
    assert lock.acquire() is True
    monkeypatch.setattr(cli, "_pid_alive", lambda pid: False)
    assert cli._daemon_owner_is_live(rec(pid=999, started_at=now, uses_flock=True), target) is True
    lock.release()

    # Free lock + live PID + aged record → reused-PID zombie → dead.
    monkeypatch.setattr(cli, "_pid_alive", lambda pid: True)
    aged = rec(pid=999, started_at=now - 3600, uses_flock=True)
    assert cli._daemon_owner_is_live(aged, target) is False

    # Free lock + live PID + young record → still starting up → alive.
    assert cli._daemon_owner_is_live(rec(pid=999, started_at=now, uses_flock=True), target) is True

    # Free lock + legacy record (no flock) → fall back to PID, never reaped.
    legacy = rec(pid=999, started_at=now - 3600, uses_flock=False)
    assert cli._daemon_owner_is_live(legacy, target) is True

    # Free lock + dead PID → dead.
    monkeypatch.setattr(cli, "_pid_alive", lambda pid: False)
    assert (
        cli._daemon_owner_is_live(rec(pid=999, started_at=now, uses_flock=True), target) is False
    )


def test_live_daemon_conflict_uses_flock(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    from omnigent import cli
    ...
    def rec(pid: int, started_at: int) -> cli._HostDaemonRecord:
        return cli._HostDaemonRecord(
            pid=pid,
            target=target,
            mode="local",
    ...
    claimer = rec(pid=111, started_at=now)

    # A different-pid record whose flock is held → a real live conflict.
    cli._write_daemon_record(rec(pid=222, started_at=now))
    ...
    # Same record, flock now free + aged → reused-PID zombie, not a conflict.
    cli._write_daemon_record(rec(pid=222, started_at=now - 3600))
...
async def test_monitor_terminates_after_record_delete(
    ...
    host._serve_task = asyncio.create_task(asyncio.Event().wait(), name="fake-serve")
    ...
    try:
        ...
        # The monitor fires once then returns on its own.
        await asyncio.wait_for(monitor, timeout=1.0)
    finally:
        if not monitor.done():
            monitor.cancel()
    # It cancelled the in-flight serve so run() breaks and tears down.
    with pytest.raises(asyncio.CancelledError):
        await host._serve_task
