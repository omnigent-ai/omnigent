...
def test_if_present_env_non_uuid_host_id_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    """The read-only sibling enforces the same host-id validity as the creator."""
    monkeypatch.setenv("OMNIGENT_HOST_ID", "superagent-databricks-host")
    monkeypatch.setenv("OMNIGENT_HOST_NAME", "supercell")

    with pytest.raises(ValueError, match="UUID"):
        load_host_identity_if_present(Path("/nonexistent/config.yaml"))


def test_if_present_config_non_uuid_host_id_raises(tmp_path: Path) -> None:
    """A bad host_id in config.yaml is rejected on the read-only path too."""
    config_path = tmp_path / "config.yaml"
    config_path.write_text(
        yaml.safe_dump({"host": {"host_id": "not-a-uuid", "name": "my-laptop"}})
    )
    ...
    with pytest.raises(ValueError, match="UUID"):
        load_host_identity_if_present(config_path)
